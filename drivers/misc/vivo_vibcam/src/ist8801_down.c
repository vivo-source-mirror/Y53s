...
static void ist8801_work_func(struct work_struct *work)
{
	...
	printk("ist8801_work_func down : rawData--== %6d\n", raw);
...
static int ist8801_input_dev_init(ist8801_data_t *p_data)
{
	...
	dev->name = IST8801_DRIVER_NAME_DOWN;
				 ...
				 struct device_attribute *attr, char *buf)
{
	   ...
	   printk(KERN_ERR"ist8801_dump_show: the down hall reg measure is 0x%02X\n", reg);
				   ...
				   const char *buf, size_t count)
{
	...
	pr_err("ist8801_regset_storebuf:%s;\n", buf);
...
static int ist8801_i2c_drv_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	hall_dev->name = DOWN_HOLL_NAME;
	err = vivo_hall_dev_register(hall_dev);
	if (err < 0) {
		pr_err("vivo hall registration failed (%d)\n", err);
		goto error_1;
	}
	mxinfo(&client->dev, "%s was initialized", IST8801_DRIVER_NAME_DOWN);
	...
	dbg("%s : %s was probed.\n", __func__, IST8801_DRIVER_NAME_DOWN);
...
static const struct i2c_device_id ist8801_i2c_drv_id_table[] = {
	{IST8801_DRIVER_NAME_DOWN, 0 },
	{ }
};


static const struct of_device_id ist8801_of_match[] = {
	{ .compatible = "isentek,ist8801,down", },
	{ },
};
...
static int __init ist8801_driver_init_down(void)
{
	printk(KERN_INFO "%s\n", __func__);
	return i2c_add_driver(&ist8801_driver);
}
module_init(ist8801_driver_init_down);

static void __exit ist8801_driver_exit_down(void)
{
	printk(KERN_INFO "%s\n", __func__);
	i2c_del_driver(&ist8801_driver);
}
module_exit(ist8801_driver_exit_down);
