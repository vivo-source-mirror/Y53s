...
/*chenhan  add for ois*/
#ifdef CONFIG_MTK_CAM_PD2083F_EX
static struct ois_vsync_info gVsyncInfo[ISP_IRQ_TYPE_AMOUNT] = { {0}, };
#endif
/*add end*/
...
irqreturn_t ISP_Irq_CAM(enum ISP_IRQ_TYPE_ENUM irq_module)
{
	...
	if (IrqStatus & SOF_INT_ST) {
		...
		gVsyncInfo[module].vsync_cnt = (u64)cur_v_cnt;
		gVsyncInfo[module].sof_timestamp_boot = ktime_get_boot_ns();
		ois_interface_dispatcher(AFICO_X_OIS_VSYNC, (void *)gVsyncInfo, "DW9781C");
		ois_interface_dispatcher(AFICO_X_OIS_VSYNC, (void *)gVsyncInfo, "LC89129");
	}
#endif
	/*add end*/
