 ...
 *****************************************************************************/
#define AF_PINCTRL_PIN_VM 0
#define AF_PINCTRL_PIN_VDDOIS 1
#define AF_PINCTRL_PINSTATE_LOW 0
#define AF_PINCTRL_PINSTATE_HIGH 1
#define AF_PINCTRL_STATE_VM_HIGH     "cam3_ldo_vm_1"
#define AF_PINCTRL_STATE_VM_LOW      "cam3_ldo_vm_0"
#define AF_PINCTRL_STATE_VDDOIS_HIGH     "cam3_ldo_vddois_1"
#define AF_PINCTRL_STATE_VDDOIS_LOW      "cam3_ldo_vddois_0"
static struct pinctrl *af_pinctrl;
static struct pinctrl_state *af_vm_high;
static struct pinctrl_state *af_vm_low;
static struct pinctrl_state *af_vddois_high;
static struct pinctrl_state *af_vddois_low;
...
static int af_pinctrl_init(struct device *pdev)
{
	...
	if (af_pinctrl == NULL) {
		...
		/* AF VM/VDDOIS pin initialization */
		af_vm_high = pinctrl_lookup_state(
				af_pinctrl, AF_PINCTRL_STATE_VM_HIGH);
		if (IS_ERR(af_vm_high)) {
			LOG_INF("Failed to init (%s)\n",
				AF_PINCTRL_STATE_VM_HIGH);
			ret = PTR_ERR(af_vm_high);
		}
		af_vm_low = pinctrl_lookup_state(
			af_pinctrl, AF_PINCTRL_STATE_VM_LOW);
		if (IS_ERR(af_vm_low)) {
			LOG_INF("Failed to init (%s)\n",
				AF_PINCTRL_STATE_VM_LOW);
			ret = PTR_ERR(af_vm_low);
		}
		
		af_vddois_high = pinctrl_lookup_state(
				af_pinctrl, AF_PINCTRL_STATE_VDDOIS_HIGH);
		if (IS_ERR(af_vddois_high)) {
			LOG_INF("Failed to init (%s)\n",
				AF_PINCTRL_STATE_VDDOIS_HIGH);
			ret = PTR_ERR(af_vddois_high);
		}
		af_vddois_low = pinctrl_lookup_state(
			af_pinctrl, AF_PINCTRL_STATE_VDDOIS_LOW);
		if (IS_ERR(af_vddois_low)) {
			LOG_INF("Failed to init (%s)\n",
				AF_PINCTRL_STATE_VDDOIS_LOW);
			ret = PTR_ERR(af_vddois_low);
		}
	}
	LOG_INF("-");
	return ret;
...
static int sub2_af_pinctrl_set(int pin, int state)
{
	int ret = 0;
	...
	switch (pin) {
	case AF_PINCTRL_PIN_VM:
		if (state == AF_PINCTRL_PINSTATE_LOW &&
				!IS_ERR(af_vm_low))
			pinctrl_select_state(af_pinctrl, af_vm_low);
		else if (state == AF_PINCTRL_PINSTATE_HIGH &&
				!IS_ERR(af_vm_high))
			pinctrl_select_state(af_pinctrl, af_vm_high);
		else
			LOG_INF("set err, pin(%d) state(%d)\n", pin, state);
		break;
	
	case AF_PINCTRL_PIN_VDDOIS:
		if (state == AF_PINCTRL_PINSTATE_LOW &&
				!IS_ERR(af_vddois_low))
			pinctrl_select_state(af_pinctrl, af_vddois_low);
		else if (state == AF_PINCTRL_PINSTATE_HIGH &&
				!IS_ERR(af_vddois_high))
			pinctrl_select_state(af_pinctrl, af_vddois_high);
		else
			LOG_INF("set err, pin(%d) state(%d)\n", pin, state);
		break;
	default:
...
#if defined(CONFIG_MTK_CAM_TD1909)
	sub2_af_pinctrl_set(AF_PINCTRL_PIN_VM, AF_PINCTRL_PINSTATE_HIGH);
	sub2_af_pinctrl_set(AF_PINCTRL_PIN_VDDOIS, AF_PINCTRL_PINSTATE_HIGH);
#endif
...
#if defined(CONFIG_MTK_CAM_TD1909)
	sub2_af_pinctrl_set(AF_PINCTRL_PIN_VM,AF_PINCTRL_PINSTATE_LOW);
	sub2_af_pinctrl_set(AF_PINCTRL_PIN_VDDOIS,AF_PINCTRL_PINSTATE_LOW);
#endif
...
static const struct of_device_id gmainaf_of_device_id[] = {
	{.compatible = "mediatek,camera_sub2_af_lens",},
	{}
};
#endif
