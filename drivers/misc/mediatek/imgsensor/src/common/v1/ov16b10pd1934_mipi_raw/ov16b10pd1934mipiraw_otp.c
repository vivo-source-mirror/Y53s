...
#define PFX "OV16B10PD1934_EEPROM_OTP"
...
#endif
static int checksum;
otp_error_code_t OV16B10PD1934_OTP_ERROR_CODE = OTP_ERROR_CODE_NORMAL;
extern MUINT32  sn_inf_main_ov16b10pd1934[13];
extern MUINT32  material_inf_main_ov16b10pd1934[4];
...
int ov16b10pd1934_otp_read(void)
{
	int i = 0;
	...
	if (is_atboot == 1) {
		LOG_INF("[lxd++]AT mode skip ov16b10pd1934_otp_read\n");
		return 1;
	}
	/* guojunzheng add end */
	...
	if (check_if_group_valid == 0x01) { /****all the data is valid****/
		...
		if ((VIVO_VENDOR_SUNNY != vivo_otp_data[0x01]) && (VIVO_VENDOR_QTECH != vivo_otp_data[0x01])) {
			OV16B10PD1934_OTP_ERROR_CODE = OTP_ERROR_CODE_MODULE_INFO_ERROR;
			LOG_INF("[lxd++]Module ID error!!!    otp_error_code:%d\n", OV16B10PD1934_OTP_ERROR_CODE);
			return 0;
		...
		if (vivo_otp_data[ModuleInfo_checksum_addr] != checksum) {
			OV16B10PD1934_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
		...
		sn_inf_main_ov16b10pd1934[0] = 0x01;
		for (i = 0; i < 12; i++) {
			sn_inf_main_ov16b10pd1934[i+1] = (MUINT32)vivo_otp_data[i + SN_addr+1];
			/*LOG_INF("sn_inf_main_ov16b10pd1934[%d] = 0x%x, vivo_otp_data[0x%x] = 0x%x\n", i+1  , sn_inf_main_ov16b10pd1934[i+1],  i +SN_addr+1, vivo_otp_data[i+SN_addr+1]);*/
		}

        /*  ***material info start****/
        material_inf_main_ov16b10pd1934[0] = (MUINT32)vivo_otp_data[0x0F];
        material_inf_main_ov16b10pd1934[1] = (MUINT32)vivo_otp_data[0x10];
        material_inf_main_ov16b10pd1934[2] = (MUINT32)vivo_otp_data[0x11];
        material_inf_main_ov16b10pd1934[3] = (MUINT32)vivo_otp_data[0x12];
        LOG_INF("material_inf_main_ov16b10pd1934[0] = 0x%x, material_inf_main_ov16b10pd1934[1] = 0x%x, material_inf_main_ov16b10pd1934[2] = 0x%x,material_inf_main_ov16b10pd1934[3] = 0x%x\n",material_inf_main_ov16b10pd1934[0],material_inf_main_ov16b10pd1934[1],material_inf_main_ov16b10pd1934[2], material_inf_main_ov16b10pd1934[3]);
               /*  ***material info end****/
		...
		if (diff_inf_macro > 480 || diff_inf_macro < 200) {  /*AF code out of range*/
			OV16B10PD1934_OTP_ERROR_CODE = OTP_ERROR_CODE_AF_OUT_OF_RANGE;
			LOG_INF("[lxd++]AF out of range error!!!   otp_error_code:%d    Module ID = 0x%0x   inf = %d    mac = %d\n", OV16B10PD1934_OTP_ERROR_CODE, vivo_otp_data[0x01], AF_inf, AF_mac);
			return 0;
		}
		#endif
		...
		{
			if(i == 0)
				LOG_INF("[lxd++]read_OV16B10PD1934_PDAF_data");
			if(i < 496)
		...
		OV16B10PD1934_OTP_ERROR_CODE = OTP_ERROR_CODE_GROUP_INVALID;
