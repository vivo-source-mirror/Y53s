...
struct mutex dma_lock_array[TASK_SCENE_SIZE];
	...
	void    *data_buffer) /* buffer for payload & dma */
{
	...
	if (data_type == AUDIO_IPI_DMA)
		mutex_lock(&dma_lock_array[task_scene]);
	...
	ret = send_message(handler, p_ipi_msg);

SEND_EXIT:
	if (data_type == AUDIO_IPI_DMA)
		mutex_unlock(&dma_lock_array[task_scene]);
