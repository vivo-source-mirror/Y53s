...
#include "../../video/mt6771/videox/disp_dts_gpio.h"
...
int mt_mt65xx_led_set_cust(struct cust_mt65xx_led *cust, int level)
{
	...
	case MT65XX_LED_MODE_CUST_LCM:
		#ifdef CONFIG_LCM_PANEL_TYPE_TFT
		LEDS_DEBUG("brightness_set_cust:backlight =%d\n", level);
		if (level == 0) {
			ret = ((cust_brightness_set) (cust->data)) (level, bl_div_hal);
			bl_brightness_hal = level;
			disp_dts_gpio_select_state(DTS_GPIO_STATE_TFT_BKG_OUT0);
			printk("MT65XX_LED_MODE_CUST_LCM lm3697 disable");
			return ret;
		} else {
			if (bl_brightness_hal == 0)
				disp_dts_gpio_select_state(DTS_GPIO_STATE_TFT_BKG_OUT1);
		}
		#endif
		ret = ((cust_brightness_set) (cust->data)) (level, bl_div_hal);
		...
		if (strcmp(cust->name, "lcd-backlight") == 0)
			bl_brightness_hal = level;
		LEDS_DEBUG("brightness_set_cust:out\n");
		return ret;
