...
void DSI_set_cmdq_serially(enum DISP_MODULE_ENUM module,
...
long lcm_vci_setting(unsigned int value)
{
	...
	if (value)
		ret = disp_dts_gpio_select_state(DTS_GPIO_STATE_LCM_VCI_EN1_GPIO);
	else
		ret = disp_dts_gpio_select_state(DTS_GPIO_STATE_LCM_VCI_EN0_GPIO);
...
long lcm_reset_setting(unsigned int value)
{
	...
	if (value)
		ret = disp_dts_gpio_select_state(DTS_GPIO_STATE_LCM_RST_OUT1_GPIO);
	else
		ret = disp_dts_gpio_select_state(DTS_GPIO_STATE_LCM_RST_OUT0_GPIO);
