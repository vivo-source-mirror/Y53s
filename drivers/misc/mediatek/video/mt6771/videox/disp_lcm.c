...
int disp_lcm_vivo_SetMipiCmd_HS(struct disp_lcm_handle *plcm, void *handle, char cmdtype, char levelsetting)
{
	...
	if (_is_lcm_inited(plcm)) {
		...
		if (lcm_drv->lcm_MipiCmd_HS) {
			...
			DISPERR("FATAL ERROR, lcm_drv->lcm_MipiCmd_HS is null\n");
			ret = -1;
		}
	} else {
...
int disp_lcm_vivo_SetMipiCmd_LP(struct disp_lcm_handle *plcm, void *handle, char cmdtype, char levelsetting)
{
	struct LCM_DRIVER *lcm_drv = NULL;
	int ret = 0;
	DISPFUNC();
	if (_is_lcm_inited(plcm)) {
		lcm_drv = plcm->drv;
		if (lcm_drv->lcm_MipiCmd_LP) {
			lcm_drv->lcm_MipiCmd_LP(handle, cmdtype, levelsetting);
		} else {
			DISPERR("FATAL ERROR, lcm_drv->lcm_MipiCmd_LP is null\n");
			ret = -1;
		}
	} else {
		DISPERR("lcm_drv cabc is null\n");
