...
#ifdef CONFIG_SPECIAL_FOR_MT6771
static __s16 vivo_bts_thermistor_conver_temp(__s32 Res)
{
	int i = 0;
	int asize = 0;
	__s32 RES1 = 0, RES2 = 0;
	...
	/* mtkts_bts_dprintk("vivo_bts_thermistor_conver_temp() :
	 * asize = %d, Res = %d\n",asize,Res);
	 */
	if (Res >= BTS_Temperature_Table[0].TemperatureR) {
		TAP_Value = -40;	/* min */
...
static __s16 vivo_ts_bts_volt_to_temp(__u32 dwVolt)
{
	__s32 TRes;
	__u64 dwVCriAP = 0;
	__s32 BTS_TMP = -100;
	...
	/* convert register to temperature */
	BTS_TMP = vivo_bts_thermistor_conver_temp(TRes);

	return BTS_TMP;
}

int vivo_get_hw_bts_temp(void)
{
	...
	/* ret = ret*1800/4096;//82's ADC power */
	mtkts_bts_dprintk("APtery output mV = %d\n", ret);
	output = vivo_ts_bts_volt_to_temp(ret);
	mtkts_bts_dprintk("BTS output temperature = %d\n", output);
	return output;
}
#endif
