			  ...
			  const struct i2c_device_id *id)
{
    ...
    //vivo xuling add for gpio i2c test
	pr_err("INIT pinS %s\n", client->adapter->name);
	
	pinctrl = devm_pinctrl_get_select(chip->dev, "active");     //enable active pin
	if (IS_ERR(pinctrl))
		pr_err("No support for active pin.\n");

	if (!(strncmp(client->adapter->name, "soc:i2c@0", 9)))
		i2c_gpio = true;
	
	if (i2c_gpio) {
		pinctrl = devm_pinctrl_get_select(chip->dev, "set");
		if (IS_ERR(pinctrl))
			pr_err("No support for set pin .\n");
	}
    //vivo xuling add end
