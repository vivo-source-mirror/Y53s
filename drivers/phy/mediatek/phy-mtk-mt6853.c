...
static void usb_phy_tuning(struct mtk_phy_instance *instance)
{
	...
	if (!instance->phy_tuning.inited) {
		...
		if (of_node) {
			...
			of_property_read_u32(of_node, "board1_version_bit",
				(u32 *) &board1_version_bit);
			if (board1_version_bit >= 0) {
				u32 board1_version_active = 0;
				char *version_state = NULL;

				of_property_read_u32(of_node, "board1_version_active",
					(u32 *) &board1_version_active);
				board1_version_active += 0x30;

				version_state = get_board_version();
				if (version_state && (version_state[5] == board1_version_active)) {
					of_property_read_u32(of_node, "u2_vrt_ref1",
						(u32 *) &instance->phy_tuning.u2_vrt_ref);
					of_property_read_u32(of_node, "u2_term_ref1",
						(u32 *) &instance->phy_tuning.u2_term_ref);
					of_property_read_u32(of_node, "u2_enhance1",
						(u32 *) &instance->phy_tuning.u2_enhance);
					of_property_read_u32(of_node, "u2_vrt_ref_host1",
						(u32 *) &instance->phy_tuning.u2_vrt_ref_host);
					of_property_read_u32(of_node, "u2_term_ref_host1",
						(u32 *) &instance->phy_tuning.u2_term_ref_host);
					of_property_read_u32(of_node, "u2_enhance_host1",
						(u32 *) &instance->phy_tuning.u2_enhance_host);

					phy_printk(K_ALET, "%s: board1 version [%d : %c]\n",
