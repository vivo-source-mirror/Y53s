			     ...
			     struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < mtkphy->phycfg->num_phys; i++) {
		...
		size += sprintf(buf + size, "%s: 0x%x\n%s: 0x%x\n%s: 0x%x\n",
				"u2_vrt_ref", instance->phy_tuning.u2_vrt_ref_real,
				"u2_term_ref", instance->phy_tuning.u2_term_ref_real,
				"u2_enhance", instance->phy_tuning.u2_enhance_real);
	}
			      ...
			      const char *buf, size_t count)
{
	...
	ptr_val++;
	*(ptr_val - 1) = 0;
	result = kstrtoint(ptr_val, 0, &val);
	...
	for (i = 0; i < mtkphy->phycfg->num_phys; i++) {
		...
		if (u2_vrt_ref) {
			instance->phy_tuning.u2_vrt_ref = val;
			instance->phy_tuning.u2_vrt_ref_host = val;
		} else if (u2_term_ref) {
			instance->phy_tuning.u2_term_ref = val;
			instance->phy_tuning.u2_term_ref_host = val;
		} else if (u2_enhance) {
			instance->phy_tuning.u2_enhance = val;
			instance->phy_tuning.u2_enhance_host = val;
		}
	}
...
static DEVICE_ATTR(phy_tune, S_IRUGO | S_IWUSR, phy_tune_show, phy_tune_store);
...
static int mtk_usb_phy_probe(struct platform_device *pdev)
{
	...
	device_create_file(dev, &dev_attr_phy_tune);
