...
static int mtk_drm_esd_check_worker_kthread(void *data)
{
	...
	static unsigned int reg_read_count;
	static unsigned int reg_read_err;
	int ret = 0;
	int i = 0;
	int recovery_flg = 0;
	...
	while (1) {
		if (vdisp->debug == LCM_ESD_DAL_PRINT) {
			DAL_Clean();
			msleep(20);
			DAL_Printf(">mipi config:%d\n", lcmpr_debug_enabled);
			DAL_Printf(">mipi read:%d, error=%d\n", reg_read_count, reg_read_err);
		}
		...
		if ((!esd_ctx->chk_active) || (!vdisp->bl_brightness_hal)) {
		...
		do {
			...
			if (vdisp->ctx->lcm_need_read_esd_once) {
				...
				if (ret && vdisp->bl_brightness_hal)
					LCD_INFO("esd read failed\n");
				break;
			}
			...
			vdisp->lcm_need_shutdown_power = false;
			...
			/*Reload touch firmware after recovery*/
			#ifdef CONFIG_LCM_ESD_CHECK_RELOAD_FW
			if (vdisp->ctx->esd_check_reload_firmware) {
