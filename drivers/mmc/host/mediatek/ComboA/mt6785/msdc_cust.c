...
int msdc_of_parse(struct platform_device *pdev, struct mmc_host *mmc)
{
	...
	if (of_get_property(np, "fast_power_off", NULL)) {
		...
		//pr_notice("\n");
		if (of_get_property(np, "PD2062F_EX", NULL)) {
			//if (*(version + 2) == '0') {
				mmc->fast_power_off = true;
				pr_notice("[msdc%d]: in fast_power_off mode", mmc->index);
			//}
		}else if (of_get_property(np, "PD2079F_EX", NULL)) {
			//if (*(version + 2) == '0') {
				mmc->fast_power_off = true;
				pr_notice("[msdc%d]: in fast_power_off mode", mmc->index);
			//}
		}else if (of_get_property(np, "PD2078F_EX", NULL)) {
			//if (*(version + 2) == '0') {
				mmc->fast_power_off = true;
				pr_notice("[msdc%d]: in fast_power_off mode", mmc->index);
			//}
		}else if (of_get_property(np, "PD2078F_EX_PDPB", NULL)) {
			//if (*(version + 2) == '0') {
				mmc->fast_power_off = true;
				pr_notice("[msdc%d]: in fast_power_off mode", mmc->index);
			//}
