...
#endif
	if (vpm_ums)
		vpm_ums_update_data_start(us, usb_pipein(pipe));
	result = usb_stor_bulk_transfer_sglist(us, pipe, scsi_sglist(srb),
				      scsi_sg_count(srb), scsi_bufflen(srb),
				      &partial);
	if (vpm_ums)
		vpm_ums_update_data_finish(us, usb_pipein(pipe),
					   partial, result);
...
int usb_stor_Bulk_transport(struct scsi_cmnd *srb, struct us_data *us)
{
	...
	usb_stor_dbg(us, "Bulk Command S 0x%x T 0x%x L %d F %d Trg %d LUN %d CL %d\n",
		     ...
		     bcb->Length);
	if (vpm_ums)
		vpm_ums_update_command_start(us, us->srb);
	result = usb_stor_bulk_transfer_buf(us, us->send_bulk_pipe,
				bcb, cbwlen, NULL);
	usb_stor_dbg(us, "Bulk command transfer result=%d\n", result);
	if (vpm_ums)
		vpm_ums_update_command_finish(us, srb, result);
	if (result != USB_STOR_XFER_GOOD)
		return USB_STOR_TRANSPORT_ERROR;
	...
	if (vpm_ums)
		vpm_ums_update_csw_finish(us, result);
