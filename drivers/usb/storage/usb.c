...
bool vpm_ums = true;
module_param(vpm_ums, bool, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(vpm_ums, "vperf monitor ums");
		...
		struct scsi_host_template *sht)
{
	...
	*pus = us = host_to_us(host);
	if (vpm_ums) {
		result = vpm_ums_register(us);
		if (result)
			pr_err("%s: %d: fail to register vpm_ums\n");
	}
