...
int fts_create_gesture_sysfs_0flash(struct device *dev)
{
    int ret = 0;
...
int fts_gesture_readdata_0flash(struct fts_ts_data *ts_data, u8 *data)
{
    int i = 0;
    int gestrue_id = 0;
    u8 pointnum = 0;
...
void fts_gesture_recovery_0flash(void)
{
    if ((ENABLE == fts_gesture_data.mode) && (ENABLE == fts_gesture_data.active)) {
        VTD("enter fts_gesture_recovery_0flash");
...
int fts_gesture_resume_0flash(struct fts_ts_data *ts_data)
{
    ...
    if (fts_gesture_data.active == DISABLE) {
        VTD("gesture in suspend is failed, no running fts_gesture_resume_0flash");
        return -EINVAL;
    }
...
int fts_gesture_init_0flash(struct fts_ts_data *ts_data)
{
    ...
    fts_create_gesture_sysfs_0flash(&ts_data->spi->dev);
    fts_gesture_data.mode = ENABLE;
