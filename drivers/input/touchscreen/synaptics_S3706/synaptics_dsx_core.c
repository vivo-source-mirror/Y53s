...
static int dsi_panel_update_hbm_aod_ud_fingerprint_for_tp(void)
{
	return 0;
}
...
static int syna_S3706_pinctrl_init(struct i2c_client *pdev)
{
	...
	pin_config.pin_active =
		pinctrl_lookup_state(pin_config.pinctrl, "ts_active");
	...
	pin_config.pin_suspend =
		pinctrl_lookup_state(pin_config.pinctrl, "ts_suspend");
...
static int syna_dclick_longpress_mode_change(int dl_switch)
{
	...
	if (dl_switch == FP_DETECT_OPEN) {
		tmp = 129;
		ret = synaptics_rmi4_reg_write(rmi4_data, switch_addr, &tmp, sizeof(tmp));
		if (ret < 0) {
...
static int syna_mode_change(int which)
{
	...
	if (which == TOUCHSCREEN_NORMAL) {
		...
		atomic_set(&rmi4_data->fp_report, 0);
		atomic_set(&rmi4_data->gesture_done, 0);
		synaptics_rmi4_resume(&rmi4_data->pdev->dev);
		if (!vivo_ts_prox_ear_detect_get()) {
			synaptics_rmi4_reset_device_simple(rmi4_data, false);
		} else {
	...
	if (which == TOUCHSCREEN_GESTURE) {
		...
		rmi4_data->sensor_sleep = 0;
		atomic_set(&rmi4_data->gesture_done, 1);
...
#define MENU_PULLING_COMPENSATION_ENABLE	0x40a
#define GAME_MODE_ADDR	0x441

static int syna_menu_pulling_compensation_set(int enable)
{
	struct synaptics_rmi4_data *rmi4_data = platform_get_drvdata(globe_pdev);
	int ret = 0;
	unsigned char temp = 0;

	ret = synaptics_rmi4_reg_read(rmi4_data, MENU_PULLING_COMPENSATION_ENABLE, &temp, 1);
		if (ret < 0) {
		VTI("failed to read addr 0x%x", MENU_PULLING_COMPENSATION_ENABLE);
		ret = -EINVAL;
		goto exit;
	}
	VTI("read MENU_PULLING_COMPENSATION_ENABLE is %d before write", temp);

	if (enable == temp) {
		VTI("value is same %d, no neet to set menu pulling compensation", temp);
		goto exit;
	}
	...
	ret = synaptics_rmi4_reg_write(rmi4_data, MENU_PULLING_COMPENSATION_ENABLE, &temp, 1);
	if (ret < 0) {
		VTI("failed to read addr 0x%x, data 0x%x", MENU_PULLING_COMPENSATION_ENABLE, temp);
		ret = -EINVAL;
		goto exit;
	}
	...
	VTI("read MENU_PULLING_COMPENSATION_ENABLE is %d after write", temp);
...
static int syna_enter_game_mode(int mode)
{
	...
	ret = synaptics_rmi4_reg_read(rmi4_data, GAME_MODE_ADDR, &temp, 1);
		if (ret < 0) {
		VTI("failed to read addr 0x%x", GAME_MODE_ADDR);
		ret = -EINVAL;
		goto exit;
	}
	VTI("before write:addr[0x%x] data[0x%x]", GAME_MODE_ADDR, temp);
	if (mode >= 0) {
	...
	ret = synaptics_rmi4_reg_write(rmi4_data, GAME_MODE_ADDR, &temp, 1);
	if (ret < 0) {
		VTI("failed to read addr 0x%x, data 0x%x", GAME_MODE_ADDR, temp);
		ret = -EINVAL;
		goto exit;
	}
	/*do force update*/
	...
	VTI("after  write:addr[0x%x] data[0x%x]", GAME_MODE_ADDR, temp);
	ret = 0;
exit:
	return ret;
}
...
static int syna_scan_rate_change (int rate)
{
	...
	VTI("gesture_change lock");
	mutex_lock(&(rmi4_data->geture_mutex));
	if (atomic_read(&(vivoTsGetVtsData()->tsState)) != TOUCHSCREEN_GESTURE || atomic_read(&rmi4_data->gesture_done) != 1) {
	...
	ret = synaptics_rmi4_reg_read(rmi4_data, scan_addr, &temp, 1);
		if (ret < 0) {
		VTI("failed to read");
		goto END;
	}
	...
	ret = synaptics_rmi4_reg_write(rmi4_data, scan_addr, &temp, 1);
	if (ret < 0) {
...
static int syna_set_edge_bit(int state)
{
	...
	if (state == 1) {
		retval = syna_menu_pulling_compensation_set(0);
	} else if (state == 0 || state == 2) {
		retval = syna_menu_pulling_compensation_set(1);
	}
	if (retval < 0) {
		VTI("failed to syna_menu_pulling_compensation_set");
		goto END;
	}
	END:
		return retval;
...
static int mdss_report_lcm_id(void)
{
	return 0x20;
}
...
static int syna_get_pinvalue(void)
{
	int ret = 0;
...
static int syna_faceDetect_startEnd(int cmd)
{
	...
	if (cmd == 1) {
		...
		temp = 0x11;
		ret = synaptics_rmi4_reg_write(rmi4_data, fdswitch_addr, &temp, 1);
		if (ret < 0) {
...
static int synaptics_rmi4_probe(struct platform_device *pdev)
{
	...
	if(strcmp(productName, "PD1938")){
		VTE("not PD1938 PANNEL RETURN");
		return MINUS_ONE;
	}
	...
	if (!strcmp(productName, "PD1938") || !strcmp(productName, "PD1949")) {
	...
	if (!strcmp(productName, "PD1938")) {
		vts_data->getTsPinValue = syna_get_pinvalue;
	} else {
		vts_data->getTsPinValue = NULL;
	}

	vivoTsFwAdd(VTS_FW_TYPE_FW, pd1938_fw, sizeof(pd1938_fw), "PD1938", VTS_FW_PIN_0, VTS_MVC_TRY, 0);
	vivoTsFwAdd(VTS_FW_TYPE_FW, pd1938_mp_fw, sizeof(pd1938_mp_fw), "PD1938", VTS_FW_PIN_1, VTS_MVC_TRY, 0);
	vivoTsFwAdd(VTS_FW_TYPE_FW, pd1949_fw, sizeof(pd1949_fw), "PD1949", VTS_FW_NOPIN_VALUE, VTS_MVC_TRY, 0);
	vivoTsFwAdd(VTS_FW_TYPE_FW, td1904_fw, sizeof(td1904_fw), "TD1905", VTS_FW_NOPIN_VALUE, VTS_MVC_TRY, 0);
...
if (1) {
	...
	VTE("create dsx_reset_workqueue");
#endif
