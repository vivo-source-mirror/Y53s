...
DECLARE_COMPLETION(s3706_prox_remove_complete);
...
int s3706_synaptics_rmi4_prox_hover_finger_en(bool enable)
{
	int retval;
...
EXPORT_SYMBOL(s3706_synaptics_rmi4_prox_hover_finger_en);
...
exit:
	complete(&s3706_prox_remove_complete);
...
{
	synaptics_3706_rmi4_new_function(&proximity_module, true);
...
{
	synaptics_3706_rmi4_new_function(&proximity_module, false);

	wait_for_completion(&s3706_prox_remove_complete);
