...
#include "ftsTime.h"
#include "ftsTool.h"
#include "../fts.h"	/* needed for the tag_ftm5 define */
...
int flushFIFO_ftm5(struct fts_ts_info *info)
{
	...
	if (ret < OK) {
		logError_ftm5(1, "%s flushFIFO_ftm5: ERROR %08X\n", tag_ftm5, ret);
		return ret;
	}

	logError_ftm5(0, "%s FIFO flushed!\n", tag_ftm5);
	return OK;
}
...
int senseOn_ftm5(struct fts_ts_info *info)
{
	...
	if (ret < OK) {
		logError_ftm5(1, "%s senseOn_ftm5: ERROR %08X\n", tag_ftm5, ret);
		return ret;
	}

	logError_ftm5(0, "%s senseOn_ftm5: SENSE ON\n", tag_ftm5);
	return OK;
}
...
int senseOff_ftm5(struct fts_ts_info *info)
{
	...
	if (ret < OK) {
		logError_ftm5(1, "%s senseOff_ftm5: ERROR %08X\n", tag_ftm5, ret);
		return ret;
	}

	logError_ftm5(0, "%s senseOff_ftm5: SENSE OFF\n", tag_ftm5);
	return OK;
}
...
int cleanUp_ftm5(struct fts_ts_info *info, int enableTouch)
{
	...
	logError_ftm5(0, "%s cleanUp_ftm5: system reset...\n", tag_ftm5);
	res = fts_system_reset_ftm5(info);
	if (res < OK)
		return res;
	if (enableTouch) {
		logError_ftm5(0, "%s cleanUp_ftm5: enabling touches...\n", tag_ftm5);
		res = senseOn_ftm5(info);	/* already enable everything */
		if (res < OK)
			return res;
		logError_ftm5(0, "%s cleanUp_ftm5: enabling interrupts...\n", tag_ftm5);
...
{
	if (((int)((id) / 8)) < size) {
		logError_ftm5(0, "%s %s: ID = %d Index = %d Position = %d !\n", tag_ftm5,
		...
		logError_ftm5(1,
			 ...
			 tag_ftm5, __func__, id, ((int)((id) / 8)), size,
			 ERROR_OP_NOT_ALLOW);
		return ERROR_OP_NOT_ALLOW;
	}
}
