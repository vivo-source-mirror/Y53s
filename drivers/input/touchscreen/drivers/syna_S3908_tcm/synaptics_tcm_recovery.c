...
DECLARE_COMPLETION(recovery_remove_complete_v2);
...
exit:
	complete(&recovery_remove_complete_v2);
...
{
	return syna_tcm_add_module_v2(&recovery_module, true);
}

void recovery_module_exit(void)
{
	syna_tcm_add_module_v2(&recovery_module, false);

	wait_for_completion(&recovery_remove_complete_v2);
