...
DECLARE_COMPLETION(report_complete_v2);

DECLARE_COMPLETION(testing_remove_complete_v2);
		...
		unsigned int num_of_reports)
{
	...
	reinit_completion(&report_complete_v2);
#else
	INIT_COMPLETION(report_complete_v2);
#endif
	...
	retval = wait_for_completion_timeout(&report_complete_v2,
...
exit:
	complete(&testing_remove_complete_v2);
...
{
	return syna_tcm_add_module_v2(&testing_module, true);
}

void testing_module_exit(void)
{
	syna_tcm_add_module_v2(&testing_module, false);

	wait_for_completion(&testing_remove_complete_v2);
