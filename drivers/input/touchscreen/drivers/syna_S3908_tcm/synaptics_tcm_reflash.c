...
#define BOOT_CONFIG_TOTAL_SIZE 128
#define BOOT_CONFIG_ONE_RECORD_SIZE 16
#define BOOT_CONFIG_SKIP_RECORD_CNT 4
...
\
	addr = le4_to_uint(tcm_hcd->boot_info.display_config_start_block); \
	addr = addr * reflash_hcd->write_block_size; \
	addr /= 2; \
	if (addr == 0xf000) { \
		LOGE(tcm_hcd->pdev->dev.parent, \
				"Display config flash address = 0xf000\n"); \
		retval = -EINVAL; \
		reset = true; \
		goto reset; \
	} else {\
		LOGN(tcm_hcd->pdev->dev.parent, \
				"Display config flash addr = 0x%x\n", addr);\
	} \
\
...
DECLARE_COMPLETION(reflash_remove_complete_v2);
...
static int reflash_update_boot_config_customer(const unsigned char *buf, int length);
...
SHOW_PROTOTYPE(reflash, fw_version_in_file)
		...
		unsigned int fw_bin_size)
{
	...
	reflash_hcd->image = fw_bin;
	reflash_hcd->image_size = fw_bin_size;
		...
		unsigned int buffer_len)
{
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"Failed to read boot config data\n");
		goto exit;
	}
	LOCK_BUFFER(reflash_hcd->read);
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"Failed reflash_get_boot_config_valid_record\n");
		goto exit;
	}
		...
		unsigned int data_len)
{
	...
	retval = reflash_update_custom_oem(data, 0, data_len);
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
...
#else
	int retval;
	retval = reflash_update_boot_config_customer(data, data_len);
	return retval;
#endif
}
...
int reflash_wrapper_get_image_fw_id(const unsigned char *fw_bin)
{
	struct syna_tcm_hcd *tcm_hcd = reflash_hcd->tcm_hcd;
	...
	retval = le4_to_uint(header->build_id);
	LOGN(tcm_hcd->pdev->dev.parent, "%d\n", retval);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	__pm_stay_awake(tcm_hcd->wakelock);
 ...
 */
static ssize_t reflash_sysfs_fw_version_in_file_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	...
	retval = reflash_wrapper_get_image_fw_id(fw_entry->data);
	...
	for (i = BOOT_CONFIG_SKIP_RECORD_CNT; i < record_length; i++) {
		if (array_all_zero(&boot_config_data[i * BOOT_CONFIG_ONE_RECORD_SIZE], record_length)) {
			if (i == BOOT_CONFIG_SKIP_RECORD_CNT) {
				printk("no valid record in the boot config area, memset 0\n");
                memset(buf, 0, length);
				return 0;
			} else {
				valid_index = i - 1;
...
static enum update_area reflash_compare_id_info(void)
{
	...
	LOGN(tcm_hcd->pdev->dev.parent,
				"Image_version=[%d], Device_version=[%d]\n", image_fw_id, device_fw_id);
	...
	if (image_fw_id != device_fw_id) {
		LOGN(tcm_hcd->pdev->dev.parent,
				"Image firmware ID not equal to device firmware ID\n");
		update_area = FIRMWARE_CONFIG;
		goto exit;
	}
#if 0
	...
	offset = snprintf(log_buf, sizeof(log_buf), "Image =[");
	for (idx = 0; idx < 16; idx++) {
		offset += snprintf(&log_buf[offset], 5, "%2x ", image_config_id[idx]);
	}
	offset += snprintf(&log_buf[offset - 1], 5, "]");
	VTI("%s", log_buf);

	memset(log_buf, 0, sizeof(log_buf));
	offset = snprintf(log_buf, sizeof(log_buf), "Device=[");
	for (idx = 0; idx < 16; idx++) {
		offset += snprintf(&log_buf[offset], 5, "%2x ", device_config_id[idx]);
	}
	offset += snprintf(&log_buf[offset - 1], 5, "]");
	VTI("%s", log_buf);
...
exit:
	complete(&reflash_remove_complete_v2);
...
int reflash_module_init(void)
{
	...
	return syna_tcm_add_module_v2(&reflash_module, true);
}

void reflash_module_exit(void)
{
	syna_tcm_add_module_v2(&reflash_module, false);

	wait_for_completion(&reflash_remove_complete_v2);
