...
int brl_gesture(struct goodix_ts_core *cd, int gesture_type)
{
	...
	ret = cd->hw_ops->read(cd, 0x1016c, cmd.buf, sizeof(cmd));
	if (ret < 0) {
		ts_err("failed read 0x1016c, %d", ret);
		goto readfail;
	}
	VTI("read from 0x1016c: %*ph", (int)sizeof(cmd), cmd.buf);
				...
				u8 *pre_buf, u32 pre_buf_len)
{
	...
	ts_event->touch_data.custom_data_buf[0] = pre_buf[3];
	ts_event->touch_data.custom_data_buf[1] = pre_buf[4];
	ts_event->touch_data.custom_data_buf[2] = pre_buf[5];
	ts_event->touch_data.custom_data_buf[3] = pre_buf[9];
