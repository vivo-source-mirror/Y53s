...
static int goodix_read_cfg_bin(struct goodix_ts_core *cd, struct goodix_cfg_bin *cfg_bin)
{
	...
	cfg_data = vts_fw_data_get(cd->vtsdev, VTS_FW_TYPE_CONFIG, &size);
	...
	VTE("goodix get config data end");
exit:
	if (firmware) {
		kfree(firmware);
		firmware = NULL;
...
static int goodix_get_config_data(struct goodix_ts_core *cd, u8 sensor_id)
{
	...
	ret = goodix_read_cfg_bin(cd, &cfg_bin);
	if (ret) {
