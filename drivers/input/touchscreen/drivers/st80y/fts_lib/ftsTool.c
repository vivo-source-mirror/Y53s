...
#include "ftsTime.h"
#include "ftsTool.h"
#include "../fts.h"	/* needed for the tag_st80y define */
...
int st80y_flushFIFO(struct fts_ts_info *info)
{
	...
	if (ret < OK) {
		logError_st80y(1, "%s st80y_flushFIFO: ERROR %08X\n", tag_st80y, ret);
		return ret;
	}

	logError_st80y(0, "%s FIFO flushed!\n", tag_st80y);
	return OK;
}
...
int st80y_senseOn(struct fts_ts_info *info)
{
	...
	if (ret < OK) {
		logError_st80y(1, "%s st80y_senseOn: ERROR %08X\n", tag_st80y, ret);
		return ret;
	}

	logError_st80y(0, "%s st80y_senseOn: SENSE ON\n", tag_st80y);
	return OK;
}
...
int st80y_senseOff(struct fts_ts_info *info)
{
	...
	if (ret < OK) {
		logError_st80y(1, "%s st80y_senseOff: ERROR %08X\n", tag_st80y, ret);
		return ret;
	}

	logError_st80y(0, "%s st80y_senseOff: SENSE OFF\n", tag_st80y);
	return OK;
}
...
int st80y_cleanUp(struct fts_ts_info *info, int enableTouch)
{
	...
	logError_st80y(0, "%s st80y_cleanUp: system reset...\n", tag_st80y);
	res = st80y_system_reset(info);
	if (res < OK)
		return res;
	if (enableTouch) {
		logError_st80y(0, "%s st80y_cleanUp: enabling touches...\n", tag_st80y);
		res = st80y_senseOn(info);	/* already enable everything */
		if (res < OK)
			return res;
		logError_st80y(0, "%s st80y_cleanUp: enabling interrupts...\n", tag_st80y);
...
{
	int i, j;
	char *buf = kzalloc(16*(row+column),GFP_KERNEL);
	if(buf == NULL){
		logError_st80y(1, "malloc fail\n");
		return;
	}
	logError_st80y(0, "%s %s\n", tag_st80y, label);
	for (i = 0; i < row; i++) {
		memset(buf, '\0', 16*(row+column));
		for (j = 0; j < column; j++)
			sprintf(buf+strlen(buf), "%5d" ,matrix[i][j]);
		logError_st80y(0, "%s %s\n", tag_st80y, buf);
		kfree(matrix[i]);
	}
	kfree(matrix);
...
{
	if (((int)((id) / 8)) < size) {
		logError_st80y(0, "%s %s: ID = %d Index = %d Position = %d !\n", tag_st80y,
		...
		logError_st80y(1,
			 ...
			 tag_st80y, __func__, id, ((int)((id) / 8)), size,
			 ERROR_OP_NOT_ALLOW);
		return ERROR_OP_NOT_ALLOW;
	}
}
