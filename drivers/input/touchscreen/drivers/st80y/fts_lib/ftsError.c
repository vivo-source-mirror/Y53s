...
/**
  ...
  * of printf
  */
void logError_st80y(int force, const char *msg, ...)
{
	if (force == 1
#ifdef DEBUG
	    || 1
...
/**
  ...
  */
int st80y_isI2cError(int error)
{
...
/**
  ...
  */
int st80y_dumpErrorInfo(struct fts_ts_info *info, u8 *outBuf, int size)
{
	int ret, i;
	u8 data[ERROR_DUMP_ROW_SIZE * ERROR_DUMP_COL_SIZE] = { 0 };
	u32 sign = 0;

	logError_st80y(0, "%s %s: Starting dump of error info...\n", tag_st80y, __func__);

	ret = st80y_writeReadU8UX(info, FTS_CMD_FRAMEBUFFER_R, BITS_16, ADDR_ERROR_DUMP,
				data, ERROR_DUMP_ROW_SIZE * ERROR_DUMP_COL_SIZE,
				DUMMY_FRAMEBUFFER);
	if (ret < OK) {
		logError_st80y(1, "%s %s: reading data ERROR %08X\n", tag_st80y, __func__,
			 ret);
		return ret;
	} else {
		if (outBuf != NULL) {
			...
			logError_st80y(0, "%s %s: error info copied in the buffer!\n",
				 tag_st80y, __func__);
		}
		logError_st80y(1, "%s %s: Error Info =\n", tag_st80y, __func__);
		st80y_u8ToU32(data, &sign);
		...
		for (i = 0; i < ERROR_DUMP_ROW_SIZE * ERROR_DUMP_COL_SIZE;
			...
			if (i % ERROR_DUMP_COL_SIZE == 0)
				logError_st80y(1, KERN_ERR "\n%s %s: %d) ", tag_st80y,
					 __func__, i / ERROR_DUMP_COL_SIZE);
			logError_st80y(1, "%02X ", data[i]);
		}
		logError_st80y(1, "\n");

		logError_st80y(0, "%s %s: dump of error info FINISHED!\n", tag_st80y,
			 __func__);
		return OK;
	}
}
...
/**
  ...
  */
int st80y_errorHandler(struct fts_ts_info *info, u8 *event, int size)
{
	int res = OK;

	if (info != NULL && event != NULL && size > 1 && event[0] ==
	    EVT_ID_ERROR) {
		logError_st80y(1, "%s Starting handling...\n", tag_st80y);
		...
		case EVT_TYPE_ERROR_ESD:	/* esd */
			...
			if (res < OK)
				logError_st80y(1,
					 "%s Error performing powercycle ERROR %08X\n",
					 tag_st80y, res);
		...
		case EVT_TYPE_ERROR_ITO_FORCETOGND:
			logError_st80y(1, "%s Force Short to GND!\n",
				 tag_st80y);
			break;
		case EVT_TYPE_ERROR_ITO_SENSETOGND:
			logError_st80y(1, "%s Sense short to GND!\n",
				 tag_st80y);
			break;
		case EVT_TYPE_ERROR_ITO_FORCETOVDD:
			logError_st80y(1, "%s Force short to VDD!\n",
				 tag_st80y);
			break;
		case EVT_TYPE_ERROR_ITO_SENSETOVDD:
			logError_st80y(1, "%s Sense short to VDD!\n",
				 tag_st80y);
			break;
		case EVT_TYPE_ERROR_ITO_FORCE_P2P:
			logError_st80y(1,
				 "%s Force Pin to Pin Short!\n",
				 tag_st80y);
			break;
		case EVT_TYPE_ERROR_ITO_SENSE_P2P:
			logError_st80y(1,
				 "%s Sense Pin to Pin Short!\n",
				 tag_st80y);
			break;
		case EVT_TYPE_ERROR_ITO_FORCEOPEN:
			logError_st80y(1, "%s Force Open !\n", tag_st80y);
			break;
		case EVT_TYPE_ERROR_ITO_SENSEOPEN:
			logError_st80y(1, "%s Sense Open !\n", tag_st80y);
			break;
		case EVT_TYPE_ERROR_ITO_KEYOPEN:
			logError_st80y(1, "%s Key Open !\n", tag_st80y);
			break;

		default:
			logError_st80y(1, "%s No Action taken!\n", tag_st80y);
			break;
		}
		logError_st80y(1, "%s handling Finished! res = %08X\n",
			 tag_st80y, res);
		return res;
	} else {
		logError_st80y(1,
			 "%s event Null or not correct size! ERROR %08X\n",
...
int addErrorIntoList(struct fts_ts_info *info, u8 *event, int size)
{
	...
	logError_st80y(0, "%s Adding error in to ErrorList...\n", tag_st80y);
...
int pollErrorList(struct fts_ts_info *info, int *event_to_search, int event_bytes)
{
	...
	logError_st80y(1, "%s Starting to poll ErrorList...\n", tag_st80y);
	...
	if (find == 1) {
		logError_st80y(1, "%s Error Found into ErrorList!\n", tag_st80y);
		return i - 1;	/* there is i++ at the end of the while */
	} else {
		logError_st80y(1, "%s Error Not Found into ErrorList! ERROR %08X\n",
			 tag_st80y, ERROR_TIMEOUT);
...
int pollForErrorType(struct fts_ts_info *info, u8 *list, int size)
{
	...
	logError_st80y(1, "%s %s: Starting to poll ErrorList... count = %d\n", tag_st80y,
		 __func__, count);
	while (find != 1 && i < count) {
		for (j = 0; j < size; j++) {
	...
	if (find == 1) {
		logError_st80y(1, "%s %s: Error Type %02X into ErrorList!\n", tag_st80y,
			 __func__, list[j]);
		return list[j];
	} else {
		logError_st80y(1,
