...
int st80y_read(struct fts_ts_info *info, u8 *outBuf, int byteToRead)
{
	...
	while (retry < I2C_RETRY && ret < OK) {
		...
		/* logError_st80y(1,"%s fts_writeCmd: attempt %d\n", tag_st80y, retry); */
	}
	if (ret < 0) {
		logError_st80y(1, "%s %s: ERROR %08X\n", tag_st80y, __func__, ERROR_BUS_R);
		return ERROR_BUS_R;
	}
	return OK;
}
...
int st80y_writeRead(struct fts_ts_info *info, u8 *cmd, int cmdLength, u8 *outBuf, int byteToRead)
{
	...
	if (ret < 0) {
		logError_st80y(1, "%s %s: ERROR %08X\n", tag_st80y, __func__, ERROR_BUS_WR);
		return ERROR_BUS_WR;
	}
	return OK;
}
...
int st80y_write(struct fts_ts_info *info, u8 *cmd, int cmdLength)
{
	...
	if (ret < 0) {
		logError_st80y(1, "%s %s: ERROR %08X\n", tag_st80y, __func__, ERROR_BUS_W);
		return ERROR_BUS_W;
	}
	return OK;
}
...
/**
  ...
  * @return OK if success, or an error code which specify the type of error
  */
int st80y_writeFwCmd(struct fts_ts_info *info, u8 *cmd, int cmdLength)
{
	int ret = -1;
	int ret2 = -1;
	int retry = 0;
	...
	while (retry < I2C_RETRY && (ret < OK || ret2 < OK)) {
		...
		retry++;
		if (ret >= 0)
			ret2 = st80y_checkEcho(info, cmd, cmdLength);
		if (ret < OK || ret2 < OK)
			msleep(I2C_WAIT_BEFORE_RETRY);
		/* logError_st80y(1,"%s fts_writeCmd: attempt %d\n", tag_st80y, retry); */
	}
	if (ret < 0) {
		logError_st80y(1, "%s st80y_writeFwCmd: ERROR %08X\n", tag_st80y,
			 ERROR_BUS_W);
		return ERROR_BUS_W;
	}
	if (ret2 < OK) {
		logError_st80y(1, "%s st80y_writeFwCmd: check echo ERROR %08X\n", tag_st80y,
			 ret2);
		return ret2;
	}
	return OK;
...
/**
  ...
  * can be achieved calling st80y_write followed by an st80y_writeRead.
		  ...
		  dataSize)
{
	...
	if (addrSize <= sizeof(u64)) {
		while (remaining > 0) {
			...
			finalCmd[0] = cmd;
			logError_st80y(0, "%s %s: addrSize = %d\n", tag_st80y, __func__,
				 addrSize);
			for (i = 0; i < addrSize; i++) {
				finalCmd[i + 1] = (u8)((address >> ((addrSize -
								     1 - i) *
								    8)) & 0xFF);
				logError_st80y(1, "%s %s: cmd[%d] = %02X\n", tag_st80y,
					 __func__, i + 1, finalCmd[i + 1]);
			}

			memcpy(&finalCmd[addrSize + 1], data, toWrite);

			if (st80y_write(info, finalCmd, 1 + addrSize + toWrite) < OK) {
				logError_st80y(1, "%s %s: ERROR %08X\n", tag_st80y,
		...
		logError_st80y(1,
			 ...
			 tag_st80y, __func__, sizeof(u64), ERROR_OP_NOT_ALLOW);
...
{
	u8 finalCmd[1 + 4];//addrSize  max 4 
	...
	while (remaining > 0) {
		...
		if (hasDummyByte == 1) {
			...
			if (st80y_writeRead(info, finalCmd, 1 + addrSize, buff, toRead +
					  1) < OK) {
				logError_st80y(1,
			...
			if (st80y_writeRead(info, finalCmd, 1 + addrSize, buff,
					  toRead) < OK) {
				logError_st80y(1,
					 "%s %s: read error... ERROR %08X\n",
					 tag_st80y,
			       ...
			       addrSize2, u64 address, u8 *data, int dataSize)
{
	...
	while (remaining > 0) {
		...
		if (st80y_write(info, finalCmd1, 1 + addrSize1) < OK) {
			logError_st80y(1, "%s %s: first write error... ERROR %08X\n",
				 tag_st80y, __func__, ERROR_BUS_W);
			return ERROR_BUS_W;
		}

		if (st80y_write(info, finalCmd2, 1 + addrSize2 + toWrite) < OK) {
			logError_st80y(1,
	...
	while (remaining > 0) {
		...
		for (i = 0; i < addrSize1; i++)
			finalCmd1[i + 1] = (u8)((address >> ((addrSize1 +
							      addrSize2 - 1 -
							      i) * 8)) & 0xFF);
		/* logError_st80y(1, "%s %s: finalCmd1[%d] =  %02X\n",
		  *	tag_st80y, __func__, i+1, finalCmd1[i + 1]); */
		...
		if (hasDummyByte == 1) {
			if (st80y_writeRead(info, finalCmd2, 1 + addrSize2, buff,
					  toRead + 1) < OK) {
				logError_st80y(1,
					 "%s %s: read error... ERROR %08X\n",
					 tag_st80y,
