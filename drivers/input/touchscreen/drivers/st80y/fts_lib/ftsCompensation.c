...
/**
  ...
  */
int st80y_requestCompensationData(struct fts_ts_info *info, u8 type)
{
	int ret = ERROR_OP_NOT_ALLOW;
	int retry = 0;

	logError_st80y(0, "%s %s: Requesting compensation data... attemp = %d\n", tag_st80y,
		 __func__, retry + 1);
	while (retry < RETRY_COMP_DATA_READ) {
		...
		if (ret < OK) {
			logError_st80y(1, "%s %s: failed at %d attemp!\n", tag_st80y,
				 __func__, retry + 1);
			retry += 1;
		} else {
			logError_st80y(0,
	...
	logError_st80y(1, "%s %s: Requesting compensation data... ERROR %08X\n", tag_st80y,
...
/**
  ...
  * @return OK if success or an error code which specify the type of error
  */
int st80y_readCompensationDataHeader(struct fts_ts_info *info, u8 type, DataHeader *header, u64 *address)
{
	...
	logError_st80y(0, "%s Type = %02X of Compensation data OK!\n", tag_st80y, type);
...
/**
  ...
  * @return OK if success or an error code which specify the type of error
  */
int st80y_readMutualSenseGlobalData(struct fts_ts_info *info, u64 *address, MutualSenseData *global)
{
	u8 data[COMP_DATA_GLOBAL];
	int ret;

	logError_st80y(0, "%s Address for Global data= %08llX\n", tag_st80y, *address);

	ret = st80y_writeReadU8UX(info, FTS_CMD_FRAMEBUFFER_R, BITS_16, *address, data,
				COMP_DATA_GLOBAL, DUMMY_FRAMEBUFFER);
	if (ret < OK) {
		logError_st80y(1, "%s %s: error while reading info data ERROR %08X\n",
			 tag_st80y, __func__, ret);
		return ret;
	}
	logError_st80y(0, "%s Global data Read !\n", tag_st80y);
	...
	logError_st80y(0, "%s force_len = %d sense_len = %d CX1 = %d\n", tag_st80y,
...
/**
  ...
  * @return OK if success or an error code which specify the type of error
  */
int st80y_readMutualSenseNodeData(struct fts_ts_info *info, u64 address, MutualSenseData *node)
{
	int ret;
	int size = node->header.force_node * node->header.sense_node;

	logError_st80y(0, "%s Address for Node data = %08llX\n", tag_st80y, address);

	node->node_data = (i8 *)kmalloc(size * (sizeof(i8)), GFP_KERNEL);

	if (node->node_data == NULL) {
		logError_st80y(1, "%s %s: can not allocate node_data... ERROR %08X",
			 tag_st80y, __func__, ERROR_ALLOC);
		return ERROR_ALLOC;
	}

	logError_st80y(0, "%s Node Data to read %d bytes\n", tag_st80y, size);
	ret = st80y_writeReadU8UX(info, FTS_CMD_FRAMEBUFFER_R, BITS_16, address,
	...
	logError_st80y(0, "%s Read node data OK!\n", tag_st80y);
...
/**
  ...
  * @return OK if success or an error code which specify the type of error
  */
int st80y_readMutualSenseCompensationData(struct fts_ts_info *info, u8 type, MutualSenseData *data)
{
	int ret;
	u64 address;
	...
	ret = st80y_requestCompensationData(info, type);
	if (ret < 0) {
	...
	ret = st80y_readCompensationDataHeader(info, type, &(data->header), &address);
	if (ret < 0) {
		logError_st80y(1, "%s %s: ERROR %08X\n", tag_st80y, __func__,
	...
	ret = st80y_readMutualSenseGlobalData(info, &address, data);
	if (ret < 0) {
		logError_st80y(1, "%s %s: ERROR %08X\n", tag_st80y, __func__,
	...
	ret = st80y_readMutualSenseNodeData(info, address, data);
	if (ret < 0) {
		logError_st80y(1, "%s %s: ERROR %08X\n", tag_st80y, __func__,
...
/**
  ...
  * @return OK if success or an error code which specify the type of error
  */
int st80y_readSelfSenseGlobalData(struct fts_ts_info *info, u64 *address, SelfSenseData *global)
{
	int ret;
	u8 data[COMP_DATA_GLOBAL];
	...
	logError_st80y(0,
		 ...
		 tag_st80y, global->header.force_node, global->header.sense_node,
		 global->f_ix1, global->s_ix1, global->f_cx1, global->s_cx1);
	logError_st80y(0, "%s max_n = %d   s_max_n = %d f_ix0 = %d  s_ix0 = %d\n",
	tag_st80y, global->f_max_n, global->s_max_n,
...
/**
  ...
  * @return OK if success or an error code which specify the type of error
  */
int st80y_readSelfSenseNodeData(struct fts_ts_info *info, u64 address, SelfSenseData *node)
{
	...
	if (data == NULL) {
		logError_st80y(1,
			 "%s %s: can not allocate memory for ix2_fm... ERROR %02X",
			 tag_st80y,
...
/**
  ...
  * @return OK if success or an error code which specify the type of error
  */
int st80y_readSelfSenseCompensationData(struct fts_ts_info *info, u8 type, SelfSenseData *data)
{
	int ret;
	u64 address;
	...
	ret = st80y_readSelfSenseGlobalData(info, &address, data);
	if (ret < 0) {
		logError_st80y(1, "%s %s: ERROR %08X\n", tag_st80y, __func__,
			 ERROR_COMP_DATA_GLOBAL);
		return ret | ERROR_COMP_DATA_GLOBAL;
	...
	ret = st80y_readSelfSenseNodeData(info, address, data);
	if (ret < 0) {
		logError_st80y(1, "%s %s: ERROR %08X\n", tag_st80y, __func__,
			 ERROR_COMP_DATA_NODE);
		return ret | ERROR_COMP_DATA_NODE;
...
int readTotMutualSenseGlobalData(struct fts_ts_info *info, u64 *address, TotMutualSenseData *global)
{
	...
	logError_st80y(0, "%s Address for Global data= %04llX\n", tag_st80y, *address);
...
int readTotMutualSenseNodeData(struct fts_ts_info *info, u64 address, TotMutualSenseData *node)
{
	...
	logError_st80y(0, "%s Address for Node data = %04llX\n", tag_st80y, address);
...
int readTotSelfSenseNodeData(struct fts_ts_info *info, u64 address, TotSelfSenseData *node)
{
	...
	if (j != toRead)
		logError_st80y(1, "%s %s: parsed a wrong number of bytes %d!=%d\n",
			 tag_st80y, __func__, j, toRead);
