...
enum cts_device_fw_reg {
    ...
    CTS_DEVICE_FW_REG_CHIP_ID    = 0x600,         //12 + 2 bytes
    //noise add end
};
...
extern int bbk_chipone_getUdd_size(struct vts_device *vtsdev, u32 *size);
extern ssize_t bbk_chipone_readUdd(struct vts_device *vtsdev, u8 *buf, size_t nbytes);
