...
int cts_tool_init(struct chipone_ts_data *cts_data)
{
	...
	ret = alloc_chrdev_region(&devid, 0, 1, "cts_tool_vivo_ts"); 
	if (ret) {
	...
	cts_class = class_create(THIS_MODULE, "cts_tool_vivo_ts");
	device_create(cts_class, NULL, MKDEV(major, 0), NULL, "cts_tool_vivo_ts"); /* /dev/cts_tool */
	if (IS_ERR(cts_class)) {
