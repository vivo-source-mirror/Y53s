...
static int disable_fw_idle_mode(struct cts_device *cts_dev, u8 *idle_mode_old)
{
    ...
    *idle_mode_old = idle_mode_reg_val;
    idle_mode_enabled = !!(idle_mode_reg_val & BIT(0));
     if (idle_mode_enabled) {
        ...
        for (i = 0; i < 100; i++) {
            ...
            ret = cts_fw_reg_readb(cts_dev, CTS_DEVICE_FW_REG_POWER_MODE, &pwr_mode);
            if (ret) {
...
static int enable_fw_idle_mode(struct cts_device *cts_dev, u8 idle_mode_old)
{
    int ret = 0;
    int i;
    bool idle_mode_enabled = false;
    ...
    //cts_lock_device(cts_dev);
    idle_mode_enabled = !!(idle_mode_old & BIT(0));
    if (idle_mode_enabled) {
        for (i = 0; i <5; i++) {
            ret = cts_fw_reg_writeb(cts_dev, CTS_DEVICE_FW_REG_FLAG_BITS,
                idle_mode_old);
    ...
    struct cts_test_param *param)
{
    ...
    if (dump_test_data_to_file) {
        r = cts_start_dump_test_data_to_file(param->test_data_filepath, !!(param->flags & CTS_TEST_FLAG_DUMP_TEST_DATA_TO_FILE_APPEND));
        if (r) {
            cts_err("Start dump test data to file failed %d", r);
        }
    }
...
int cts_test_reset_pin(struct cts_device *cts_dev, struct cts_test_param *param)
{
    ...
    {
        r = wait_fw_to_normal_work(cts_dev);
        if (r) {
        ...
        struct cts_test_param *param)
{
    ...
    for (i = 0; i < 5; i++) {
        idle_mode_err = disable_fw_idle_mode(cts_dev, &idle_mode);
        if (idle_mode_err) {
            cts_err("Disable fw idle mode failed %d", idle_mode_err);
            mdelay(1);
        }
        else 
            break;
    ...
    if (i >= 5) {
        cts_err("Disable id mode failed");
        ret = -EIO;
        goto unlock_device;
    }
...
unlock_device:
    if (idle_mode_err == 0) 
        enable_fw_idle_mode(cts_dev, idle_mode);
    cts_unlock_device(cts_dev);
