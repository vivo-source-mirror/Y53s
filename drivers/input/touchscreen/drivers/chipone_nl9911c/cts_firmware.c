...
#else
const struct cts_firmware *cts_request_firmware(u32 hwid, u16 fwid, u16 curr_firmware_ver)
{

    struct cts_firmware *firmware;
    struct firmware array_info;
    ...
    array_info.data = vts_fw_data_get(cts_data->vtsdev, VTS_FW_TYPE_FW, &fw_size);
    array_info.size = fw_size;
    firmware->size = fw_size;
    if (!is_firmware_size_valid(firmware)) {
        cts_info("File size: %zu invalid",  firmware->size);
        goto err_close_file;
    }
    ...
    memcpy(firmware->data,array_info.data,fw_size);
