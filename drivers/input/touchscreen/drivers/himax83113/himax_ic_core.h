...
struct hx_guest_info {
	...
	#define ic_adr_sonoff_psw_lb             0x31
	#define ic_adr_sonoff_psw_ub             0x32
	#define ic_cmd_ahb_access_direction_read 0x00
	#define ic_cmd_conti	                 0x31
	#define ic_cmd_incr4	                 0x10
	#define ic_cmd_sonoff_psw_lb                0x27
	#define ic_cmd_sonoff_psw_ub                0x95
	...
	#define zf_data_adc_cfg_2             0x10007B00
...
struct ic_operation {
	...
	uint8_t  adr_sonoff_psw_lb[1];
	uint8_t  adr_sonoff_psw_ub[1];
	uint8_t  data_ahb_access_direction_read[1];
	uint8_t  data_conti[1];
	uint8_t  data_incr4[1];
	uint8_t  data_sonoff_psw_lb[1];
	uint8_t  data_sonoff_psw_ub[1];
...
struct himax_core_fp {
	...
	int (*fp_Calculate_CRC_with_AP)(unsigned char *FW_content, int CRC_from_FW, int mode);
	...
	int (*fp_check_remapping)(void);
	void (*fp_reload_fw_clear_register)(void);
	bool (*fp_check_chip_version)(void);
	int (*fp_read_ic_trigger_type)(void);
	int (*fp_read_xfer_status)(void);
	...
	bool (*fp_get_DSRAM_data)(uint8_t *info_data, bool DSRAM_flag);//27
#endif
	...
	int (*fp_usb_detect_set)(int cable_config);
