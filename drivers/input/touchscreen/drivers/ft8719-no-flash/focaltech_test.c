...
static int focaltech_enter_factory(void)
{
	...
	ret = fts_read_reg_byte_V2(DEVICE_MODE_ADDR, &RunState);
	VTI("ReCode=%d", ret);
	if (ret >= ERROR_CODE_OK) {
		if (((RunState >> 4) & 0x07) == 0x04) {  /*factory */
			ret = ERROR_CODE_OK;
		} else {
			ret = fts_write_reg_byte_V2(DEVICE_MODE_ADDR, 0x40);
			if (ret >= ERROR_CODE_OK) {
...
static int focaltech_exit_factory(void)
{
	...
	ret = fts_write_reg_byte_V2(0x00, 0);
	if (ret < 0) {
		VTI("exit factary mode failed");
	} else {
		VTI("read factary mode done");
		ret = fts_read_reg_byte_V2(0x00, &val);
		if (ret < 0) {
			VTI("exit factary mode failed");
		} else {
...
static int focaltech_startscan_tp(void)
{
	...
	ret = fts_read_reg_byte_V2(DEVICE_MODE_ADDR, &read_val);
	if (ret >= 0) {
		read_val |= 0x80;
		ret = fts_write_reg_byte_V2(DEVICE_MODE_ADDR, read_val);
		if (ret >= 0) {
...
static unsigned char focaltech_read_data(unsigned char Freq, unsigned char LineNum, int ByteNum, short *pRevBuffer)
{
	...
	ReCode = fts_write_reg_byte_V2(REG_LINE_NUM, LineNum); /*Set row addr; */
	...
	ReCode = fts_read_V2(I2C_wBuffer, 1, pReadData, ByteNum);
...
static int focaltech_getRawOrDiffdata(int mode, short *data)
{
	...
	vts_property_get(fts_data_V2->vtsdev, VTS_PROPERTY_SENSOR_TX_NUM, &tx_num);
	vts_property_get(fts_data_V2->vtsdev, VTS_PROPERTY_SENSOR_RX_NUM, &rx_num);

	VTI("set read rawdata or diffdata, mode = %d", mode);
	ret = fts_write_reg_byte_V2(RAW_DIFF_STATE_ADDR, (u8)mode);
	if (ret < 0) {
		VTE("set read rawdata or diffdata failed");
	} else {
		ret = fts_read_reg_byte_V2(RAW_DIFF_STATE_ADDR, (u8 *)&val);
		if (ret < 0) {
