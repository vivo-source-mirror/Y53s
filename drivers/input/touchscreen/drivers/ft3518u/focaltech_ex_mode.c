...
static int fts_ex_mode_switch(enum _ex_mode mode, u8 value)
{
	...
	switch (mode) {
	case MODE_GLOVE:
		ret = ft3518u_fts_write_reg(FTS_REG_GLOVE_MODE_EN, m_val);
		if (ret < 0) {
			FTS_ERROR("MODE_GLOVE switch to %d fail", m_val);
		}
		break;
	case MODE_COVER:
		ret = ft3518u_fts_write_reg(FTS_REG_COVER_MODE_EN, m_val);
		if (ret < 0) {
			FTS_ERROR("MODE_COVER switch to %d fail", m_val);
		}
		break;
	case MODE_CHARGER:
		ret = ft3518u_fts_write_reg(FTS_REG_CHARGER_MODE_EN, m_val);
		if (ret < 0) {
				   ...
				   struct device_attribute *attr, char *buf)
{
	...
	ft3518u_fts_read_reg(FTS_REG_GLOVE_MODE_EN, &val);
				   ...
				   struct device_attribute *attr, char *buf)
{
	...
	mutex_lock(&input_dev->mutex);
	ft3518u_fts_read_reg(FTS_REG_COVER_MODE_EN, &val);
