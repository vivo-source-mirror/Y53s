	...
	enum FW_STATUS *fw_sts)
{
	...
	if (upg->func->upgspec_version >= UPGRADE_SPEC_V_1_0)
		...
		cmd_len = 2;
	ret = ft3518u_fts_write(cmd, cmd_len);
	if (ret < 0) {
	...
	ret = ft3518u_fts_read(cmd, 1, val, 2);
	if (ret < 0) {
...
static int fts_fwupg_reset_to_boot(struct fts_upgrade *upg)
{
	...
	ret = ft3518u_fts_write_reg(reg, FTS_UPGRADE_AA);
	if (ret < 0) {
	...
	ret = ft3518u_fts_write_reg(reg, FTS_UPGRADE_55);
	if (ret < 0) {
...
static u16 fts_crc16_calc_host(const u8 *pbuf, u32 length)
{
	u16 ecc = 0;
	u32 i = 0;
	u32 j = 0;
	...
	u32 ecc_length)
{
	...
	ret = ft3518u_fts_write(cmd, FTS_ROMBOOT_CMD_ECC_NEW_LEN);
	if (ret < 0) {
	...
	for (i = 0; i < FTS_ECC_FINISH_TIMEOUT; i++) {
		msleep(1);
		ret = ft3518u_fts_read(cmd, 1, val, 1);
		if (ret < 0) {
...
static int fts_pram_ecc_cal_xor(void)
{
	...
	ret = ft3518u_fts_read_reg(FTS_ROMBOOT_CMD_ECC, &reg_val);
	if (ret < 0) {
		FTS_ERROR("read pramboot ecc fail");
		return ret;
	}
...
static int fts_pram_write_buf(struct fts_upgrade *upg, const u8 *buf, u32 len)
{
	int ret = 0;
	u32 i = 0;
	u32 j = 0;
	...
	for (i = 0; i < packet_number; i++) {
		...
		ret = ft3518u_fts_write(packet_buf, packet_len + cmdlen);
		if (ret < 0) {
...
static int fts_pram_init(void)
{
	...
	ret = ft3518u_fts_read(wbuf, 1, &reg_val, 1);
	if (ret < 0) {
...
static bool fts_fwupg_check_fw_valid(void)
{
	...
	ret = ft3518u_fts_wait_tp_to_valid();
	if (ret < 0) {
...
/************************************************************************
* Name: ft3518u_fwupg_reset_in_boot
* Brief: RST CMD(07), reset to romboot(bootloader) in boot environment
* Input:
* Output:
* Return: return 0 if success, otherwise return error code
***********************************************************************/
int ft3518u_fwupg_reset_in_boot(void)
{
	int ret = 0;
	u8 cmd = FTS_CMD_RESET;
...
/************************************************************************
* Name: ft3518u_fwupg_enter_into_boot
* Brief: enter into boot environment, ready for upgrade
* Input:
* Output:
* Return: return 0 if success, otherwise return error code
***********************************************************************/
int ft3518u_fwupg_enter_into_boot(void)
{
	int ret = 0;
	bool fwvalid = false;
	bool state = false;
	...
	if (fwvalid) {
		...
		ret = ft3518u_fwupg_reset_in_boot();
		if (ret < 0) {
	...
	int retries_delay)
{
	...
	for (i = 0; i < retries; i++) {
		...
		ret = ft3518u_fts_read(&cmd , 1, val, FTS_CMD_FLASH_STATUS_LEN);
...
/************************************************************************
 * Name: ft3518u_fwupg_erase
 * Brief: erase flash area
 * Input: delay - delay after erase
 * Output:
 * Return: return 0 if success, otherwise return error code
 ***********************************************************************/
int ft3518u_fwupg_erase(u32 delay)
{
	int ret = 0;
	u8 cmd = 0;
	bool flag = false;
 ...
 ***********************************************************************/
int ft3518u_fwupg_ecc_cal(u32 saddr, u32 len)
{
	int ret = 0;
	u32 i = 0;
	...
	for (i = 0; i < packet_num; i++) {
		...
		ret = ft3518u_fts_write(wbuf, cmdlen);
		if (ret < 0) {
	...
	ret = ft3518u_fts_read(wbuf, 1, val, ecc_len);
	if (ret < 0) {
 ...
 ***********************************************************************/
int ft3518u_flash_read_buf(u32 saddr, u8 *buf, u32 len)
{
	int ret = 0;
	u32 i = 0;
	u32 packet_number = 0;
	...
	for (i = 0; i < packet_number; i++) {
		...
		wbuf[2] = BYTE_OFF_8(addr);
		wbuf[3] = BYTE_OFF_0(addr);
		ret = ft3518u_fts_write(wbuf, FTS_CMD_READ_LEN);
		if (ret < 0) {
		...
		ret = ft3518u_fts_read(NULL, 0, buf + offset, packet_len);
		if (ret < 0) {
...
static int fts_flash_read(u32 addr, u8 *buf, u32 len)
{
	...
	ret = ft3518u_fwupg_enter_into_boot();
	if (ret < 0) {
		FTS_ERROR("enter into pramboot/bootloader fail");
		goto read_flash_err;
	}

	ret = ft3518u_flash_read_buf(addr, buf, len);
	if (ret < 0) {
...
#if FTS_ESDCHECK_EN
	ft3518u_fts_esdcheck_switch(DISABLE);
#endif

	if (force) {
		if (upg->func->force_upgrade) {
			ret = upg->func->force_upgrade(buf, len);
		} else {
		...
		if (upg->func->lic_upgrade) {
			ret = upg->func->lic_upgrade(buf, len);
		} else {
			FTS_INFO("lic_upgrade function is null, no upgrade");
		}
#endif
		if (upg->func->upgrade) {
			ret = upg->func->upgrade(buf, len);
		} else {
...
err_bin:
#if FTS_ESDCHECK_EN
	ft3518u_fts_esdcheck_switch(ENABLE);
#endif
...
int vivo_fw_update(const struct firmware *fw)
{
	int ret = 0;
	u32 fw_file_len = 0;
	u8 *fw_file_buf = NULL;
	...
	FTS_INFO("start upgrade with fw bin");
	if ((!upg) || (!upg->func) || !upg->ts_data || !fw) {
		FTS_ERROR("upgrade/func/ts_data is null");
		return -EINVAL;
	}
...
static int fts_lic_get_vid_in_tp(u16 *vid)
{
	...
	ret = ft3518u_fts_read_reg(FTS_REG_VENDOR_ID, &val[0]);
	if (ft3518u_fts_data->ic_info.is_incell)
		ret = ft3518u_fts_read_reg(FTS_REG_MODULE_ID, &val[1]);
	if (ret < 0) {
...
static int fts_lic_get_ver_in_tp(u8 *ver)
{
	...
	ret = ft3518u_fts_read_reg(FTS_REG_LIC_VER, ver);
	if (ret < 0) {
...
static int fts_param_get_ver_in_tp(u8 *ver)
{
	...
	ret = ft3518u_fts_read_reg(FTS_REG_IDE_PARA_VER_ID, ver);
	if (ret < 0) {
...
static int fts_param_get_ver_in_host(struct fts_upgrade *upg, u8 *ver)
{
	if ((!upg) || (!upg->func) || (!upg->fw) || (!ver)) {
		FTS_ERROR("ft3518u_fts_data/upgrade/func/fw/ver is NULL");
		return -EINVAL;
	}
...
static int fts_param_ide_in_host(struct fts_upgrade *upg)
{
	...
	if ((!upg) || (!upg->func) || (!upg->fw)) {
		FTS_ERROR("ft3518u_fts_data/upgrade/func/fw is NULL");
		return -EINVAL;
	}
...
static int fts_param_ide_in_tp(u8 *val)
{
	...
	ret = ft3518u_fts_read_reg(FTS_REG_IDE_PARA_STATUS, val);
	if (ret < 0) {
 ...
 * Return: return 0 if success, otherwise return error code
 ***********************************************************************/
int ft3518u_fwupg_upgrade(struct fts_upgrade *upg)
{
	int ret = 0;
...
static int fts_fwupg_get_vendorid(struct fts_upgrade *upg, int *vid)
{
	...
	fwvalid = fts_fwupg_check_fw_valid();
	if (fwvalid) {
		ret = ft3518u_fts_read_reg(FTS_REG_VENDOR_ID, &vendor_id);
		if (upg->ts_data->ic_info.is_incell)
			ret = ft3518u_fts_read_reg(FTS_REG_MODULE_ID, &module_id);
	} else {
