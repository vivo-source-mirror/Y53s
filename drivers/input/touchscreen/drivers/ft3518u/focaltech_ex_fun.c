...
#define PROC_NAME                               "focaltech_vivo_ts"
				   ...
				   size_t count, loff_t * ppos)
{
	...
	case PROC_WRITE_REGISTER:
		ret = ft3518u_fts_write_reg(writebuf[1], writebuf[2]);
		if (ret < 0) {
	...
	case PROC_READ_DATA:
		if ((buflen > 1) && (buflen <= FTS_MAX_COMMMAND_LENGTH)) {
			writelen = buflen - 1;
	...
	case PROC_WRITE_DATA:
		writelen = buflen - 1;
		ret = ft3518u_fts_write(writebuf + 1, writelen);
		if (ret < 0) {
	...
	case PROC_ENTER_TEST_ENVIRONMENT:
		...
		if (0 == writebuf[1]) {
			ft3518u_fts_enter_test_environment(0);
		} else {
			ft3518u_fts_enter_test_environment(1);
		}
		break;
	...
	case PROC_WRITE_DATA_DIRECT:
		writelen = buflen - 1;
		ret =
			ft3518u_fts_spi_transfer_direct(writebuf + 1, writelen, NULL, 0);
		if (ret < 0) {
				  ...
				  size_t count, loff_t * ppos)
{
...
#if FTS_ESDCHECK_EN
	ft3518u_fts_esdcheck_proc_busy(1);
#endif

	switch (proc->opmode) {
	case PROC_READ_REGISTER:
		num_read_chars = 1;
		ret = ft3518u_fts_read_reg(proc->cmd[0], &readbuf[0]);
		if (ret < 0) {
	...
	case PROC_READ_DATA:
		...
		ret = ft3518u_fts_read(proc->cmd, proc->cmd_len, readbuf, num_read_chars);
		if (ret < 0) {
	...
	case PROC_READ_DATA_DIRECT:
		num_read_chars = buflen;
		ret =
			ft3518u_fts_spi_transfer_direct(proc->cmd, proc->cmd_len,
...
#if FTS_ESDCHECK_EN
	ft3518u_fts_esdcheck_proc_busy(0);
#endif
			  ...
			  int *eof, void *data)
{
	...
	case PROC_READ_DATA_DIRECT:
		num_read_chars = buflen;
		ret = ft3518u_fts_spi_transfer_direct(proc->cmd, proc->cmd_len, readbuf, num_read_chars);
		if (ret < 0) {
			FTS_ERROR("PROC_READ_DATA_DIRECT read error");
			goto proc_read_err;
		}
...
int ft3518u_fts_create_apk_debug_channel(struct fts_ts_data *ts_data)
{
	struct ftxxxx_proc *proc = &ts_data->proc;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0))
...
void ft3518u_fts_release_apk_debug_channel(struct fts_ts_data *ts_data)
{
	struct ftxxxx_proc *proc = &ts_data->proc;
...
{
	struct input_dev *input_dev = ft3518u_fts_data->vtsdev->idev;
	ssize_t count = 0;
...
{
	ssize_t count = 0;
	struct irq_desc *desc = irq_to_desc(ft3518u_fts_data->irq);
				  ...
				  const char *buf, size_t count)
{
	...
	if (FTS_SYSFS_ECHO_ON(buf)) {
		...
		ft3518u_fts_data->fw_is_running = false;
	} else if (FTS_SYSFS_ECHO_OFF(buf)) {
		FTS_INFO("[EX-FUN]set to fw mode");
		ft3518u_fts_data->fw_is_running = true;
	}
	mutex_unlock(&input_dev->mutex);
	FTS_FUNC_EXIT();
				 ...
				 struct device_attribute *attr, char *buf)
{
	...
	FTS_FUNC_ENTER();
	mutex_lock(&input_dev->mutex);
	if (true == ft3518u_fts_data->fw_is_running) {
				...
				struct device_attribute *attr, char *buf)
{
	...
	ft3518u_fts_esdcheck_proc_busy(1);
#endif
	ret = ft3518u_fts_read_reg(FTS_REG_FW_VER, &fwver);
#if FTS_ESDCHECK_EN
	ft3518u_fts_esdcheck_proc_busy(0);
#endif
	...
	if (rw_op.len < 0) {
		...
		if (RWREG_OP_READ == rw_op.type) {
			if (rw_op.len == 1) {
				u8 reg, val;
				reg = rw_op.reg & 0xFF;
				rw_op.res = ft3518u_fts_read_reg(reg, &val);
				rw_op.val = val;
			} else {
				char reg;
				reg = rw_op.reg & 0xFF;

				rw_op.res = ft3518u_fts_read(&reg, 1, rw_op.opbuf, rw_op.len);
			}
			...
			if (rw_op.len == 1) {
				...
				reg = rw_op.reg & 0xFF;
				val = rw_op.val & 0xFF;
				rw_op.res = ft3518u_fts_write_reg(reg, val);
			} else {
				rw_op.res = ft3518u_fts_write(rw_op.opbuf, rw_op.len);
			}
			if (rw_op.res < 0) {
...
// 	mutex_lock(&input_dev->mutex);
// 	ft3518u_fts_upgrade_bin(fwname, 0);
// 	mutex_unlock(&input_dev->mutex);
...
// 	mutex_lock(&input_dev->mutex);
// 	ft3518u_fts_upgrade_bin(fwname, 1);
// 	mutex_unlock(&input_dev->mutex);
				   ...
				   struct device_attribute *attr, char *buf)
{
	...
	struct input_dev *input_dev = ts_data->vtsdev->idev;
				  ...
				  char *buf)
{
	...
	for (i = 0; i < ft3518u_fts_data->pnt_buf_size; i++) {
		count += snprintf(buf + count, PAGE_SIZE, "%02x ", ft3518u_fts_data->point_buf[i]);
	}
	count += snprintf(buf + count, PAGE_SIZE, "\n");
	mutex_unlock(&input_dev->mutex);
