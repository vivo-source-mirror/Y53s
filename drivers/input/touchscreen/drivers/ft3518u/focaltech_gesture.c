...
static void fts_gesture_report(struct input_dev *input_dev, int gesture_id)
{
    ...
    /* report event key */
    if (gesture != -1) {
		vts_report_event_down(ft3518u_fts_data->vtsdev, gesture);
		vts_report_event_up(ft3518u_fts_data->vtsdev, gesture);
    }
}

extern void fts_report_fod(struct fts_ts_data *ts_data);
...
int ft3518u_fts_gesture_readdata(struct fts_ts_data *ts_data, u8 *data)
{
    ...
    ret = ft3518u_fts_read_reg(FTS_REG_GESTURE_EN, &buf[0]);
    ...
    ret = ft3518u_fts_read(&buf[2], 1, &buf[2], FTS_GESTURE_DATA_LEN - 2);
    if (ret < 0) {
    ...
    if (gesture->gesture_id == 0x26) {
        FTS_INFO("gesture is fod, need read fod info");
        fts_report_fod(ts_data);
    }
    ...
    vts_report_coordinates_set(ts_data->vtsdev, gesture->coordinate_x, gesture->coordinate_y, gesture->point_num);
...
void ft3518u_fts_gesture_recovery(struct fts_ts_data *ts_data)
{
    if (ts_data->gesture_mode && ts_data->suspended) {
        ...
        ft3518u_fts_write_reg(0xD1, 0xFF);
        ft3518u_fts_write_reg(0xD2, 0xFF);
        ft3518u_fts_write_reg(0xD5, 0xFF);
        ft3518u_fts_write_reg(0xD6, 0xFF);
        ft3518u_fts_write_reg(0xD7, 0xFF);
        ft3518u_fts_write_reg(0xD8, 0xFF);
        ft3518u_fts_write_reg(FTS_REG_GESTURE_EN, ENABLE);
    }
}
...
int ft3518u_fts_gesture_resume(struct fts_ts_data *ts_data)
{
    ...
    for (i = 0; i < 5; i++) {
        ft3518u_fts_write_reg(FTS_REG_GESTURE_EN, DISABLE);
        msleep(1);
        ft3518u_fts_read_reg(FTS_REG_GESTURE_EN, &state);
        if (state == DISABLE)
            break;
