...
*****************************************************************************/
#define FTS_DEBUG VTD
#define FTS_FUNC_ENTER() VTD("%s: Enter\n", __func__)
#define FTS_FUNC_EXIT() VTD("%s: Exit(%d)\n", __func__, __LINE__)
#define FTS_INFO VTI
#define FTS_ERROR VTE

#define VTD_BUF(prefix_str, buf, len) do { \
		VTD("%s", prefix_str); \
		if(vts_is_debug()) \
			print_hex_dump(KERN_DEBUG, "VIVO_TS_DBG: ", DUMP_PREFIX_OFFSET, 32, 1, buf, len, false); \
	} while (0)
