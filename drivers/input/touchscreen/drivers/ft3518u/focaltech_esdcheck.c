...
int idc_esdcheck_lcderror(struct fts_ts_data *ts_data)
{
	...
	if ((tp_need_recovery == 1) && (lcd_need_reset == 0)) {
		...
		ft3518u_fts_tp_state_recovery(ts_data);
	}

	ret = ft3518u_fts_read_reg(FTS_REG_ESD_SATURATE, &val);
	if (ret < 0) {
...
static bool get_chip_id(struct fts_ts_data *ts_data)
{
	...
	for (i = 0; i < 3; i++) {
		...
		if (ret < 0) {
			...
			if ((idh == chip_id) || (ft3518u_fts_check_cid(ts_data, idh) == 0)) {
				break;
			} else {
...
static bool get_flow_cnt(struct fts_ts_data *ts_data)
{
	...
	ret = ft3518u_fts_read(&reg_addr, 1, &reg_value, 1);
	if (ret < 0) {
...
static int esdcheck_algorithm(struct fts_ts_data *ts_data)
{
	...
	ret = ft3518u_fts_read_reg(reg_addr, &reg_value);
	if (ret < 0) {
		fts_esdcheck_data.nack_cnt++;
