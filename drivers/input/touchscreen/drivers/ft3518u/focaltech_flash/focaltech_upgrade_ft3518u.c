...
***********************************************************************/
static int fts_ft5452_upgrade(const u8 * buf, u32 len)
{
	int ret = 0;
	u32 start_addr = 0;
	u8 cmd[4] = { 0 };
	...
	ret = ft3518u_fts_write(cmd, FTS_CMD_DATA_LEN_LEN);
	if (ret < 0) {
		FTS_ERROR("data len cmd write fail");
		goto fw_reset;
	}

	ret = ft3518u_fwupg_erase(FTS_REASE_APP_DELAY);
	if (ret < 0) {
	...
	/* write app */
	start_addr = upgrade_func_ft3518u.appoff;
	ecc_in_host = ft3518u_flash_write_buf(start_addr, buf, len, 1);
	if (ecc_in_host < 0) {
	...
	for (i = 0; i < FTS_RETRIES_ECC_CAL; i++) {
		...
		ft3518u_fts_read(wbuf, 1, reg_val, 2);
...
struct upgrade_func upgrade_func_ft3518u = {
	.ctype = {0x81, 0x89},
	.fwveroff = 0x010E,
	.fwcfgoff = 0x1FFB0,
	.appoff = 0x0000,
