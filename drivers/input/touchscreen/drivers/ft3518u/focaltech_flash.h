...
#define FTS_FLASH_PACKET_LENGTH                     64     /* max=128 */
...
struct upgrade_func {
    ...
    int (*upgrade)(const u8 *, u32);
    int (*get_hlic_ver)(u8 *);
    int (*lic_upgrade)(u8 *, u32);
    int (*param_upgrade)(const u8 *, u32);
    int (*force_upgrade)(const u8 *, u32);
};
...
*****************************************************************************/
extern struct upgrade_func upgrade_func_ft3518u;
...
int ft3518u_flash_write_buf(u32 saddr, const u8 *buf, u32 len, u32 delay);
