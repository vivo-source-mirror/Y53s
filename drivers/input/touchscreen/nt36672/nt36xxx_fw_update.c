...
if(0) {
	...
	if (*(fw_entry->data + FW_BIN_VER_OFFSET) + *(fw_entry->data + FW_BIN_VER_BAR_OFFSET) != 0xFF) {
		...
		VTE("FW_VER=0x%02X, FW_VER_BAR=0x%02X", *(fw_entry->data+FW_BIN_VER_OFFSET), *(fw_entry->data+FW_BIN_VER_BAR_OFFSET));
		return -EINVAL;
	}
...
void Boot_Update_Firmware(struct work_struct *work)
{
	...
	if (ret) {
		VTE("update_firmware_request failed. (%d)", ret);
		goto out;
	}
...
out:
	...
	ic_fw_version = get_chip_FW_Ver();
}

int hand_update_firmware(const struct firmware *firmware)
{
	const struct firmware *tmp_fw = fw_entry;
