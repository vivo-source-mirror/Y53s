...
//      return CFG_ERR_OPEN_FILE;
    ...
    while (1) /*find section */
    {
		...
		/* xlg */
		if (!strncmp(g_st_ini_file_data[g_used_key_num].pKeyName, "IC_Type", 7))
		{
			memcpy(g_ScreenSetParam.ic_type, g_st_ini_file_data[g_used_key_num].pKeyValue, 
				g_st_ini_file_data[g_used_key_num].iKeyValueLen);

			FTS_TEST_DBG("[%s]=%s", g_st_ini_file_data[g_used_key_num].pKeyName,
						g_ScreenSetParam.ic_type);
		}
		/* xlg end */
        ret = g_used_key_num;
