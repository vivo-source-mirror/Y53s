...
static int fts_test_get_testparam_from_ini(char *config_name)
{
    ...
    VTD("ini_size = %d ", inisize);
    if (inisize <= 0)
    {
        VTE("%s ERROR:Get firmware size failed",  __func__);
        return -EIO;
    }
    ...
    if (NULL == pcfiledata)
    {
        VTE("fts_malloc failed in function:%s",  __func__);
        return -1;
    }
    ...
    {
        VTE(" - ERROR: fts_test_read_ini_data failed" );
        fts_free(pcfiledata);
        pcfiledata = NULL;
    ...
    {
        VTD("fts_test_read_ini_data successful");
    }
...
static int fts_test_entry(char *ini_file_name)
{
    ...
    if (ret < 0)
    {
        VTE("focaltech_test_main_init() error.");
        goto TEST_ERR;
    }
    ...
    VTD("ini_file_name = %s", cfgname);

    fts_test_funcs();

    if (fts_test_get_testparam_from_ini(cfgname) <0)
    {
        VTE("get testparam from ini failure");
        goto TEST_ERR;
    }

	if ( (g_global.ic_type == _FT8006) &&
		 !(strncmp(g_ScreenSetParam.ic_type, "FT8006", 6)))
	{
		FTS_TEST_DBG("test FT8006");
	}
	else if ((g_global.ic_type == _FT8736) &&
			 !(strncmp(g_ScreenSetParam.ic_type, "FT8736", 6)))
	{
		FTS_TEST_DBG("test FT8736");
	}
	else
	{
...
int fts_test_init(struct i2c_client *client)
{
    ...
    if (0 != err)
    {
        VTE( "[focal] %s() - ERROR: sysfs_create_group() failed.",  __func__);
        sysfs_remove_group(&client->dev.kobj, &fts_test_attribute_group);
        return -EIO;
    }
    else
    {
        VTD("[focal] %s() - sysfs_create_group() succeeded.", __func__);
    }
