...
#if 1//(FTS_CHIP_TEST_TYPE & FT8736_TEST)
...
unsigned char FT8736_TestItem_EnterFactoryMode(void)
{
    ...
    int i ;
    SysDelay(150);
    VTD("Enter factory mode...");
    for (i = 1; i <= iRedo; i++)
    {
        ReCode = EnterFactory();
        if (ERROR_CODE_OK != ReCode)
        {
            VTE("Failed to Enter factory mode...");
            if (i < iRedo)
            {
                SysDelay(50);
                continue;
        ...
        {
            VTD(" success to Enter factory mode...");
            break;
        }
...
unsigned char FT8736_TestItem_RawDataTest(bool * bTestResult)
{
    ...
    if ( ERROR_CODE_OK != ReCode )
    {
        VTE("Failed to get Raw Data!! Error Code: %d",  ReCode);
        return ReCode;
    }
    //----------------------------------------------------------Show RawData
    ...
    for (iRow = 0; iRow<g_stSCapConfEx.ChannelXNum; iRow++)
    {
        ...
        {
            VTD("%5d, ", m_RawData[iRow][iCol]);
        }
    }
    VTD("\nKeys:  ");
    for ( iCol = 0; iCol < g_stSCapConfEx.KeyNumTotal; iCol++ )
    {
        VTD("%5d, ",  m_RawData[g_stSCapConfEx.ChannelXNum][iCol]);
    }
    ...
    for (iRow = 0; iRow<g_stSCapConfEx.ChannelXNum; iRow++)
    {
        ...
        for (iCol = 0; iCol < g_stSCapConfEx.ChannelYNum; iCol++)
        {
            ...
            //VTD("Node=(%d,  %d), Get_value=%d,  Set_Range=(%d, %d) \n",  iRow+1, iCol+1, iValue, RawDataMin, RawDataMax);
            if (iValue < RawDataMin || iValue > RawDataMax)
            {
                btmpresult = false;
                VTE("rawdata test failure. Node=(%d,  %d), Get_value=%d,  Set_Range=(%d, %d) ",  \
                               iRow+1, iCol+1, iValue, RawDataMin, RawDataMax);
            }
        }
    }
...
unsigned char FT8736_TestItem_CbTest(bool* bTestResult)
{
    ...
    VTD("\nKeys:  ");
    {
        for ( iCol = 0; iCol < g_stSCapConfEx.KeyNumTotal; iCol++ )
        {
            VTD("%3d, ",  m_CBData[g_stSCapConfEx.ChannelXNum][iCol]);
    ...
    for (iRow = 0; iRow < g_stSCapConfEx.ChannelXNum; iRow++)
    {
        for (iCol = 0; iCol < g_stSCapConfEx.ChannelYNum; iCol++)
        {
            ...
            {
                btmpresult = false;
                VTE("CB test failure. Node=(%d,  %d), Get_value=%d,  Set_Range=(%d, %d) ",  \
                               iRow+1, iCol+1, iValue, iMinValue, iMaxValue);
            }
        }
    }
    ...
    if (bIncludeKey)
    {
        ...
        for ( iCol = 0; iCol < g_stSCapConfEx.KeyNumTotal; iCol++)
        {
            ...
            iMaxValue = g_stCfg_Incell_DetailThreshold.CBTest_Max[iRow][iCol];
            iValue = focal_abs(m_CBData[iRow][iCol]);
            // VTD("Node1=(%d,  %d), Get_value=%d,  Set_Range=(%d, %d) \n",  iRow+1, iCol+1, iValue, iMinValue, iMaxValue);
            if (iValue < iMinValue || iValue > iMaxValue)
            {
                btmpresult = false;
                VTE("rawdata test failure. Node=(%d,  %d), Get_value=%d,  Set_Range=(%d, %d) ",  \
...
static unsigned char GetChannelNum(void)
{
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        if (ReCode == ERROR_CODE_OK)
        {
            if (0 < rBuffer[0] && rBuffer[0] < 80)
            {
                ...
                {
                    VTE("Failed to get Channel X number, Get num = %d, UsedMaxNum = %d",
        ...
        {
            VTE("Failed to get Channel X number");
            SysDelay(150);
        }
    }
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        if (ReCode == ERROR_CODE_OK)
        {
            if (0 < rBuffer[0] && rBuffer[0] < 80)
            {
                ...
                {
                    VTE("Failed to get Channel Y number, Get num = %d, UsedMaxNum = %d",
        ...
        {
            VTE("Failed to get Channel Y number");
            SysDelay(150);
        }
    }
    ...
    VTD("CH_X = %d, CH_Y = %d, Key = %d",  g_stSCapConfEx.ChannelXNum ,g_stSCapConfEx.ChannelYNum, g_stSCapConfEx.KeyNum );
    return ReCode;
}
/************************************************************************
* Name: GetRawData
