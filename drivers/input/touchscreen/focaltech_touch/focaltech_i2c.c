 ...
 *****************************************************************************/
extern struct vivo_ts_struct *g_vts_data;
	...
	if (readlen > 0) {
		if (writelen > 0) {
			...
			while (retries < 5) {
				...
				if (ret != 2) {
					VTE("[IIC]: i2c_transfer(read) error, ret=%d!!", ret);
					mdelay(10);
				}
			...
			if (retries >= 5) {
				/*fts_reset_proc(200); */
				g_vts_data->iicErrorNum++;
				VTE("############## %s: i2c_transfer(read) error, and reset = %d", __func__, retries);
			}
...
int fts_i2c_write(struct i2c_client *client, char *writebuf, int writelen)
{
	...
	if (writelen > 0) {
		...
		while (retries < 5) {
			...
			mutex_unlock(&(vivoTsGetVtsData()->lcmResumeAndProximityMutex));
			if (ret != 1) {
				VTE("%s: i2c_transfer(write) error, ret=%d", __func__, ret);
				mdelay(10);
			}
			if (ret > 0)
				break;
		...
		if (retries >= 5) {
			/*fts_reset_proc(200); */
			g_vts_data->iicErrorNum++;
			VTE(" ############## %s: i2c_transfer(write) error, and reset = %d", __func__, retries);
		}
	}
	mutex_unlock(&i2c_rw_access);
