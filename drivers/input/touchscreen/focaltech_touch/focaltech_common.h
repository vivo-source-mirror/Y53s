...
/*#define FTS_CHIP_IDC            1//((FTS_CHIP_TYPE & FLAGBIT(FLAG_IDC_BIT)) == FLAGBIT(FLAG_IDC_BIT)) */
#define TO_IC_SER(t)			(t & FLAGBITS(0, FLAG_ICSERIALS_LEN - 1))
...
#define ENABLE                              1
#define DISABLE                             0
#define INTERVAL_READ_REG                   80  /*interval time per read reg unit:ms */
#define TIMEOUT_READ_REG                    400 /*timeout of read reg unit:ms */
/*register address */
...
extern struct fts_global g_global;
/* i2c communication */
...
#define FTS_FUNC_ENTER() printk(KERN_ERR "VIVO_TS_INF^%d^[%s]: Enter\n", __LINE__, __func__)
#define FTS_FUNC_EXIT()  printk(KERN_ERR "VIVO_TS_INF^%d^[%s]: Exit\n", __LINE__, __func__)
#else
