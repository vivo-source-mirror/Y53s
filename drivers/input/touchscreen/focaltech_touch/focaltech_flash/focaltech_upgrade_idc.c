...
int fts_ctpm_upgrade_idc_init(struct i2c_client *client, u32 dw_lenth)
{
	...
	VTI("[UPGRADE]**********Upgrade setting Init**********");
...
int fts_ctpm_start_fw_upgrade(struct i2c_client *client)
{
	...
	VTI("[UPGRADE]**********send 0xAA and 0x55 to FW, start upgrade**********");
...
bool fts_ctpm_check_run_state(struct i2c_client *client, int rstate)
{
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
		...
		VTI("[UPGRADE]: run state = %d", cstate);
...
int fts_ctpm_pramboot_ecc(struct i2c_client *client)
{
	...
	VTI("[UPGRADE]******read out pramboot checksum******");
	auc_i2c_write_buf[0] = 0xcc;
	msleep(2);
	fts_i2c_read(client, auc_i2c_write_buf, 1, reg_val, 1);
	if (reg_val[0] != upgrade_ecc) {/*pramboot checksum != host checksum, upgrade fail */
		VTE("[UPGRADE]: checksum fail : pramboot_ecc = %X, host_ecc = %X!!", reg_val[0], upgrade_ecc);
		return -EIO;
	}
	VTI("[UPGRADE]: checksum success : pramboot_ecc = %X, host_ecc = %X!!", reg_val[0], upgrade_ecc);
	msleep(100);
...
int fts_ctpm_upgrade_ecc(struct i2c_client *client, u32 startaddr, u32 length)
{
	...
	if (reg_val[0] != upgrade_ecc) {/*if check sum fail, upgrade fail */
		VTE("[UPGRADE]: ecc error! FW=%02x upgrade_ecc=%02x!!", reg_val[0], upgrade_ecc);
		return -EIO;
	}

	VTI("[UPGRADE]: ecc success : FW=%02x upgrade_ecc=%02x!!", reg_val[0], upgrade_ecc);
...
int fts_ctpm_erase_flash(struct i2c_client *client)
{
	...
	for (i = 0; i < 100; i++) {
		...
		reg_val[0] = reg_val[1] = 0x00;
		fts_i2c_read(client, auc_i2c_write_buf, 1, reg_val, 2);
		FTS_DEBUG("erase status:%x%x", reg_val[0], reg_val[1]);
...
int fts_ctpm_write_pramboot_for_idc(struct i2c_client *client, u32 length, u8 *readbuf)
{
	...
	upgrade_ecc = 0;
	VTI("[UPGRADE]**********write pramboot to pram**********");
...
int fts_ctpm_write_app_for_idc(struct i2c_client *client, u32 length, u8 *readbuf)
{
	...
	VTI("[UPGRADE]**********write app to flash**********");
	...
	for (j = 0; j < packet_number; j++) {
		...
		if (j < (packet_number - 1)) {
			...
			writelenght = ((length) % FTS_PACKET_LENGTH) ? ((length) % FTS_PACKET_LENGTH) : FTS_PACKET_LENGTH;
		}
...
u16 crc_calc(u8 *pbt_buf, u32 addr, u16 length)
{
	...
	VTI("[UPGRADE][ECC] : %04x  data:%04x, len:%04x!!", (addr), data_word(pbt_buf, (addr)), length);
	for (i = 0; i < length; i++) {
...
bool ecc_check(u8 *pbt_buf, u32 star_addr, u32 len, u16 ecc_addr)
{
	...
	VTI("[UPGRADE][ECC] : ecc1 = %x, cal_ecc = %x", ecc1, cal_ecc);
	if (ecc1 != cal_ecc) {
		VTE("[UPGRADE][ECC] : ecc error!!");
		return false;
	}
	return true;
}
...
bool fts_check_app_bin_valid_idc(u8 *pbt_buf)
{
	...
	if ((data_word(pbt_buf, APP_VERIF_ADDR + APP_LEN_H) + data_word(pbt_buf, APP_VERIF_ADDR + APP_LEN_H_NE)) != 0xFFFF) {
		VTE("[UPGRADE]APP.BIN Verify- Length2 XOR error");
		return false;
	}
	len +=  ((u32)data_word(pbt_buf, APP_VERIF_ADDR + APP_LEN_H) << 16);
#endif
	VTI("%x %x %x %x", APP2_START, len, ((u32)data_word(pbt_buf, APP_VERIF_ADDR + APP_LEN_H) << 16), data_word(pbt_buf, APP_VERIF_ADDR + APP_LEN));
	len -= APP2_START;
