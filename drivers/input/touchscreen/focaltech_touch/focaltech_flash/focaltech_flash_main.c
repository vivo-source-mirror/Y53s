...
struct upgrade_fw fw_list[] = {
    {FTS_GET_VENDOR_ID, fw_file, sizeof(fw_file)},
    {FTS_VENDOR_1_ID, fw_file2, sizeof(fw_file2)},
    {FTS_VENDOR_2_ID, fw_file3, sizeof(fw_file3)},
};
...
u16 fts_host_pram_ecc(u8 *pbuf, u16 length)
{
    u16 ecc = 0;
    u16 i, j;
...
#if FTS_8719_UPGRADE_EN
    ecc_in_host = fts_host_pram_ecc(buf, len);
#endif
...
int fts_fwupg_get_boot_state(struct i2c_client *client, enum FW_STATUS *fw_sts)
{
    ...
    u8 cmd[4] = { 0 };
    u8 val[2] = { 0 };
    struct ft_chip_t ids = chip_types/*fts_data->ic_info.ids*/;
    struct fts_upgrade *upg = fwupgrade;
...
static int fts_lic_get_vid_in_tp(struct i2c_client *client, u16 *vid)
{
    ...
    if (1/*fts_data->ic_info.is_incell*/)
...
int fts_fwupg_get_vendorid(struct fts_ts_data *ts_data, u16 *vid)
{
    ...
    if (fwvalid) {
        ...
        if (1/*ts_data->ic_info.is_incell*/)
...
/*        upg->fw = fw->fw_file;
		...
		upg->fw = fw_buffer;
        upg->fw_length = fwSize;
        upg->lic = fw_buffer;
        upg->lic_length = fwSize;
 ...
 *  Return:
 *****************************************************************************/
static int fts_fwupg_init(void)
{
    int i = 0;
    int j = 0;
    int ic_stype = 0x0F;
...
{
    int ret = 0;
    struct fts_ts_data *ts_data = fts_wq_data;

    FTS_INFO("fw upgrade work function");
    
    ret = fts_fwupg_init();
    if (ret < 0) {
        FTS_ERROR("fts_fwupg_init, can't upgrade");
    } else {
