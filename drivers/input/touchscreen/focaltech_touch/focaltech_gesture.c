...
static ssize_t fts_gesture_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	count = snprintf(buf, 1023, "Gesture Mode: %s\n", fts_gesture_data.mode ? "On" : "Off");
	count += snprintf(buf + count, 1023, "Reg(0xD0) = %d\n", val);
...
static ssize_t fts_gesture_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (FTS_SYSFS_ECHO_ON(buf)) {
		VTI("[GESTURE]enable gesture");
		fts_gesture_data.mode = ENABLE;
	} else if (FTS_SYSFS_ECHO_OFF(buf)) {
		VTI("[GESTURE]disable gesture");
...
int fts_create_gesture_sysfs(struct i2c_client *client)
{
	...
	if (ret != 0) {
		VTE("[GESTURE]fts_gesture_mode_group(sysfs) create failed!");
...
int fts_gesture_init(struct input_dev *input_dev, struct i2c_client *client)
{
	...
	fts_gesture_data.mode = 0;	/*eleven 1->0 */
...
static void fts_check_gesture(struct input_dev *input_dev, int gesture_id)
{
	...
	case GESTURE_DOWN:
		envp[0] = "GESTURE=DOWN";
		gesture = KEY_WAKEUP_SWIPE;/*KEY_GESTURE_DOWN; */
		break;
	case GESTURE_DOUBLECLICK:
		envp[0] = "GESTURE=DOUBLE_CLICK";
		gesture = KEY_WAKEUP;/*KEY_POWER; */
		break;
	case GESTURE_O:
		envp[0] = "GESTURE=O";
...
{
	static u8 buf[FTS_GESTRUE_POINTS * 4] = { 0 };
	int ret = -1;
	int i = 0;
	int gestrue_id = 0;
	int read_bytes = 0;
	...
	memset(buf, 0, FTS_GESTRUE_POINTS * 4);
	...
	if (ret < 0) {
		VTE("[GESTURE]Read gesture header data failed!!");
		FTS_FUNC_EXIT();
		return ret;
	}

	VTI("[GESTURE]chip_types.chip_idh=%d", chip_types.chip_idh);

	/* FW recognize gesture */
	if (chip_types.chip_idh == 0x54 || chip_types.chip_idh == 0x58 || chip_types.chip_idh == 0x86 || chip_types.chip_idh == 0x87 || chip_types.chip_idh == 0x64 || chip_types.chip_idh == 0x80) {
		memcpy(fts_gesture_data.header, buf, FTS_GESTRUE_POINTS_HEADER);
		gestrue_id = buf[0];
		pointnum = buf[1];
		read_bytes = ((int)pointnum) * 4 + 2;
		buf[0] = FTS_REG_GESTURE_OUTPUT_ADDRESS;
		VTI("[GESTURE]PointNum=%d", pointnum);
		ret = fts_gesture_read_buffer(client, buf, read_bytes);
		if (ret < 0) {
			VTE("[GESTURE]Read gesture touch data failed!!");
			FTS_FUNC_EXIT();
			return ret;
		}
		...
		if (pointnum < MAX_POINT) {
			for (i = pointnum; i < MAX_POINT; i++) {
				fts_gesture_data.coordinate_x[i] = 65535;
				fts_gesture_data.coordinate_y[i] = 65535;
			}

			if (GESTURE_O_CW == gestrue_id)
				fts_gesture_data.coordinate_y[MAX_POINT - 1] = 16;
			else
				fts_gesture_data.coordinate_y[MAX_POINT - 1] = 32;

			 fts_gesture_data.header[1] = MAX_POINT;
		 }
	...
	if (0x24 == buf[0]) {
		gestrue_id = 0x24;
		fts_check_gesture(fts_input_dev, gestrue_id);
		VTI("[GESTURE]%d check_gesture gestrue_id", gestrue_id);
		FTS_FUNC_EXIT();
		return MINUS_ONE;
	}
	...
	ret = fts_gesture_read_buffer(client, buf, read_bytes);
	if (ret < 0) {
		VTE("[GESTURE]Driver recognize gesture - Read gesture touch data failed!!");
		FTS_FUNC_EXIT();
		return ret;
	}
...
/*		fts_i2c_write_reg(i2c_client, 0xd1, 0xff);
		...
		/*Enable FTS_REG_GESTURE_EN bit, all gesture is available */
	...
	if (i >= 5) {
		VTE("[GESTURE]Enter into gesture(suspend) failed!\n");
		FTS_FUNC_EXIT();
		return MINUS_ONE;
	}
#endif
	...
	VTI("[GESTURE]Enter into gesture(suspend) successfully!");
...
int fts_gesture_resume(struct i2c_client *client)
{
	...
	if (i >= 5) {
		VTE("[GESTURE]Clear gesture(resume) failed!\n");
	}
