...
static void esd_process(u8 *writebuf, int buflen, bool flag)
{
	if (flag) {
		if ((writebuf[1] == 0xFC) && (writebuf[2] == 0x55) && (buflen == 0x03)) {
			/* Upgrade command */
			VTD("[ESD]: Upgrade command(%x %x %x)!!", writebuf[0], writebuf[1], writebuf[2]);
			fts_esdcheck_switch (DISABLE);
		} else if ((writebuf[1] == 0x00) && (writebuf[2] == 0x40) && (buflen == 0x03)) {
			/* factory mode bit 4 5 6 */
			VTD("[ESD]: Entry factory mode(%x %x %x)!!", writebuf[0], writebuf[1], writebuf[2]);
			fts_esdcheck_switch (DISABLE);
		} else if ((writebuf[1] == 0x00) && (writebuf[2] == 0x00) && (buflen == 0x03)) {
			/* normal mode bit 4 5 6 */
			VTD("[ESD]: Exit factory mode(%x %x %x)!!", writebuf[0], writebuf[1], writebuf[2]);
		...
		if ((writebuf[1] == 0x07) && (buflen == 0x02)) {
			VTD("[ESD]: Upgrade finish-trigger reset(07)(%x %x)!!", writebuf[0], writebuf[1]);
			fts_esdcheck_switch (ENABLE);
		} else {
...
#endif
*/
	if (writebuf[0] == PROC_WRITE_REGISTER) {  /* write operation */
		if (writebuf[1] == 0x00 && writebuf[2] == 0x40) {  /* enter factory mode */
			if (fts_wq_data != NULL) {
				atomic_set(&fts_wq_data->ft_apk_open, 1);
				VTI("apk open and enter factory mode!");
			}
		} else if (writebuf[1] == 0x00 && writebuf[2] == 0x00) {  /* exit factory mode */
			if (fts_wq_data != NULL) {
				atomic_set(&fts_wq_data->ft_apk_open, 0);
				VTI("apk close and exit to normal mode!");
			}
		}
	}
    ...
    switch (proc_operate_mode)
    {
		...
		fts_irq_enable();
		if (ret < 0) {
			VTE("[APK]: upgrade failed!!");
		}
	}
	break;

	case PROC_SET_TEST_FLAG:
		VTD("[APK]: PROC_SET_TEST_FLAG = %x!!", writebuf[1]);
#if FTS_ESDCHECK_EN
		if (writebuf[1] == 1/*0*/) {
			fts_esdcheck_switch (DISABLE);
		} else {
	...
	case PROC_HW_RESET:
		...
		snprintf(tmp, 1023, "%s", writebuf + 1);
		tmp[buflen - 1] = '\0';
...
static ssize_t fts_debug_read(struct file *filp, char __user *buff, size_t count, loff_t *ppos)
{
	...
	case PROC_UPGRADE:
		...
		if (ret < 0)
			num_read_chars = snprintf(buf, 1023, "%s", "get fw version failed.\n");
		else
			num_read_chars = snprintf(buf, 1023, "current fw version:0x%02x\n", regvalue);
		break;
	case PROC_READ_REGISTER:
		readlen = 1;
...
*/
	...
	case PROC_UPGRADE:
	{
		char upgrade_file_path[FILE_NAME_LENGTH];
		memset(upgrade_file_path, 0, sizeof(upgrade_file_path));
		snprintf(upgrade_file_path, 1023, "%s", writebuf + 1);
		upgrade_file_path[buflen - 1] = '\0';
		VTD("%s\n", upgrade_file_path);
		fts_irq_disable();
#if FTS_ESDCHECK_EN
...
int fts_create_apk_debug_channel(struct i2c_client *client)
{
	...
	if (NULL == fts_proc_entry) {
		VTE("Couldn't create proc entry!");
		return -ENOMEM;
	} else {
		VTI("Create proc entry success!");
...
static int fts_hex_to_str(char *hex, int iHexLen, char *ch, int *iChLen)
{
	...
	ch[iCharLen] = '\0';
	*iChLen = iCharLen;
	VTD("iCharLen: %d, iChLen: %d in function:%s!!\n\n", iCharLen, *iChLen, __func__);
	return 0;
}
...
static ssize_t fts_tprwreg_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (!g_rwreg_result.op) {
		if (g_rwreg_result.result == 0) {
			count = snprintf(buf, 1023, "Read %02X: %02X\n", g_rwreg_result.reg, g_rwreg_result.value);
		} else {
			count = snprintf(buf, 1023, "Read %02X failed, ret: %d\n", g_rwreg_result.reg,  g_rwreg_result.result);
		}
	} else {
		if (g_rwreg_result.result == 0) {
			count = snprintf(buf, 1023, "Write %02X, %02X success\n", g_rwreg_result.reg,  g_rwreg_result.value);
		} else {
			count = snprintf(buf, 1023, "Write %02X failed, ret: %d\n", g_rwreg_result.reg,  g_rwreg_result.result);
		}
	}
	mutex_unlock(&fts_input_dev->mutex);
...
static ssize_t fts_tprwreg_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (num_read_chars != 2) {
		if (num_read_chars != 4) {
			VTE("please input 2 or 4 character");
			goto error_return;
		}
	}
	...
	if (0 != retval) {
		VTE("%s() - ERROR: Could not convert the given input to a number. The given input was: \"%s\"", __FUNCTION__, buf);
		goto error_return;
	}
#if FTS_ESDCHECK_EN
	fts_esdcheck_proc_busy(1);
	...
	if (2 == num_read_chars) {
		...
		if (g_rwreg_result.result < 0) {
			VTE("Could not read the register(0x%02x)", regaddr);
		} else {
			VTI("the register(0x%02x) is 0x%02x", regaddr, regvalue);
		...
		if (g_rwreg_result.result < 0) {
			VTE("Could not write the register(0x%02x)", regaddr);

		} else {
			VTI("Write 0x%02x into register(0x%02x) successful", regvalue, regaddr);
			g_rwreg_result.result = 0;
		}
	}
#if FTS_ESDCHECK_EN
...
/************************************************************************
 * Name: fts_fwupgradelcd_show
 * Brief:  no
 * Input: device, device attribute, char buf
 * Output: no
 * Return: EPERM
 ***********************************************************************/
static ssize_t fts_fwupgradelcd_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	/* place holder for future use */
	return -EPERM;
}
...
/************************************************************************
 * Name: fts_fwupgradelcd_store
 * Brief:  upgrade from lcd cfg.bin
 * Input: device, device attribute, char buf, char count
 * Output: no
 * Return: char count
 ***********************************************************************/
static ssize_t fts_fwupgradelcd_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	char fwname[FILE_NAME_LENGTH];
	struct i2c_client *client = container_of(dev, struct i2c_client, dev);
	...
	fts_esdcheck_switch (DISABLE);
#endif
	if (fts_updatefun_curr.upgrade_with_lcd_cfg_bin_file)
		fts_updatefun_curr.upgrade_with_lcd_cfg_bin_file(client, fwname);
#if FTS_ESDCHECK_EN
	fts_esdcheck_switch (ENABLE);
#endif
	fts_irq_enable();
...
static ssize_t fts_forceupgrade_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	/* place holder for future use */
	return -EPERM;
}

static ssize_t fts_forceupgrade_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	char fwname[FILE_NAME_LENGTH];
	struct i2c_client *client = container_of(dev, struct i2c_client, dev);
	...
	fts_esdcheck_switch (DISABLE);
#endif
        fts_force_upgrade_bin_8006(client, fwname);
#if FTS_ESDCHECK_EN
	fts_esdcheck_switch (ENABLE);
#endif
	fts_irq_enable();
...
static ssize_t fts_driverversion_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	count = snprintf(buf, 1023, FTS_DRIVER_VERSION "\n");
...
static ssize_t fts_module_config_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	count += snprintf(buf, 1023, "FTS_CHIP_TYPE: \t\t\t%04X\n", (unsigned int)(g_global.ic_type));
	count += snprintf(buf + count, 1023, "FTS_DEBUG_EN: \t\t\t%s\n", FTS_DEBUG_EN ? "ON" : "OFF");
#if defined(FTS_MT_PROTOCOL_B_EN)
	count += snprintf(buf + count, 1023, "TS_MT_PROTOCOL_B_EN: \t\t%s\n", FTS_MT_PROTOCOL_B_EN ? "ON" : "OFF");
#endif
	count += snprintf(buf + count, 1023, "FTS_GESTURE_EN: \t\t%s\n", FTS_GESTURE_EN ? "ON" : "OFF");
	count += snprintf(buf + count, 1023, "FTS_ESDCHECK_EN: \t\t%s\n", FTS_ESDCHECK_EN ? "ON" : "OFF");
#if defined(FTS_PSENSOR_EN)
	count += snprintf(buf + count, 1023, "FTS_PSENSOR_EN: \t\t%s\n", FTS_PSENSOR_EN ? "ON" : "OFF");
#endif
	count += snprintf(buf + count, 1023, "FTS_GLOVE_EN: \t\t\t%s\n", FTS_GLOVE_EN ? "ON" : "OFF");
	count += snprintf(buf + count, 1023, "FTS_COVER_EN: \t\t%s\n", FTS_COVER_EN ? "ON" : "OFF");
	count += snprintf(buf + count, 1023, "FTS_CHARGER_EN: \t\t\t%s\n", FTS_CHARGER_EN ? "ON" : "OFF");

	count += snprintf(buf + count, 1023, "FTS_REPORT_PRESSURE_EN: \t\t%s\n", FTS_REPORT_PRESSURE_EN ? "ON" : "OFF");
	count += snprintf(buf + count, 1023, "FTS_FORCE_TOUCH_EN: \t\t%s\n", FTS_FORCE_TOUCH_EN ? "ON" : "OFF");

	count += snprintf(buf + count, 1023, "FTS_TEST_EN: \t\t\t%s\n", FTS_TEST_EN ? "ON" : "OFF");
	count += snprintf(buf + count, 1023, "FTS_APK_NODE_EN: \t\t%s\n", FTS_APK_NODE_EN ? "ON" : "OFF");
	count += snprintf(buf + count, 1023, "FTS_POWER_SOURCE_CUST_EN: \t%s\n", FTS_POWER_SOURCE_CUST_EN ? "ON" : "OFF");
	count += snprintf(buf + count, 1023, "FTS_AUTO_UPGRADE_EN: \t\t%s\n", FTS_AUTO_UPGRADE_EN ? "ON" : "OFF");
...
static ssize_t fts_show_log_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (FTS_SYSFS_ECHO_ON(buf)) {
		VTD("enable show log info/error");
		g_show_log = 1;
	} else if (FTS_SYSFS_ECHO_OFF(buf)) {
		VTD("disable show log info/error");
		g_show_log = 0;
	}
	mutex_unlock(&fts_input_dev->mutex);
	return count;
...
/*
static ssize_t fts_imei_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    char data[100];
    mutex_lock(&fts_input_dev->mutex);
    memcpy(data, buf, 16);
    ...
    FTS_DEBUG("data:%s count:%d", data, 16);
    fts_write_imei_flash(data, 0, 16);
    mutex_unlock(&fts_input_dev->mutex);
    return count;
}

static ssize_t fts_imei_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    char tmp[256];
    int count = 0;
    char imei[17] = { 0 };
    ...
    fts_read_imei_flash(imei, 0, 16);
    imei[16] = '\0';
    count += sprintf(tmp + count, "imei:%x %s\n", imei[0], imei);
...
static DEVICE_ATTR(fts_upgrade_lcd, S_IRUGO | S_IWUSR, fts_fwupgradelcd_show, fts_fwupgradelcd_store);
static DEVICE_ATTR(fts_force_upgrade, S_IRUGO | S_IWUSR, fts_forceupgrade_show, fts_forceupgrade_store);
/*static DEVICE_ATTR(fts_imei, S_IRUGO|S_IWUSR, fts_imei_show, fts_imei_store);*/
...
static struct attribute *fts_attributes[] = {
	...
	&dev_attr_fts_upgrade_lcd.attr,
	&dev_attr_fts_force_upgrade.attr,
/*	&dev_attr_fts_imei.attr,*/
	NULL
};
...
int fts_create_sysfs(struct i2c_client *client)
{
	...
	if (0 != err) {
		VTE("[EX]: sysfs_create_group() failed!!");
		sysfs_remove_group(&client->dev.kobj, &fts_attribute_group);
		return -EIO;
	} else {
		VTI("[EX]: sysfs_create_group() succeeded!!");
	}
	return err;
}
/************************************************************************
