...
#include "firmware/PD1831F_EX_vts_fw.h"
		...
		unsigned char *response_code, unsigned int polling_delay_ms)
{
	...
	if (tcm_hcd->er_disable_irq_flag) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"early_resume disable irq is open, ignore the command and return fail\n");
		return -EINVAL;
	}
	
	if (atomic_read(&tcm_hcd->host_downloading) && (command != CMD_DOWNLOAD_CONFIG)) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"host_downloading ignore the command and return fail\n");
		return -EINVAL;
	}
	...
	for (idx = 0; idx < chunks; idx++) {
		...
		VTI("write %d data to buffer", xfer_length + 1);
	...
	if (en) {
		...
		if (tcm_hcd->er_disable_irq_flag) {
			LOGI(tcm_hcd->pdev->dev.parent,
					"early_resume disable irq is open, no need to enable irq\n");
			retval = -1;
			goto exit;
		}
...
/*
 * call in early_resume state as before lcd reset to disable irq.
 */
static int syna_tcm_enable_or_disable_irq(bool en)
{
	int retval = 0;
	struct syna_tcm_hcd *tcm_hcd = mod_pool.tcm_hcd;
	...
	if (tcm_hcd != NULL) {
		if (en == false) {
			tcm_hcd->er_disable_irq_flag = true;
			complete(&response_complete);
		}
		retval = syna_tcm_enable_irq(tcm_hcd, en, true);
	} else {
		VTE("Enable or Disable irq Failed!");
		retval = -EPERM;
	}
...
{
	int retval;
	VIVO_TS_LOG_INF("Switch to %s mode\n", (mode == FW_MODE_BOOTLOADER) ? "Bootloader" : "Application");
...
static int syna_tcm_reset(struct syna_tcm_hcd *tcm_hcd, bool hw, bool update_wd)
{
	...
	resp_buf = NULL;
	resp_buf_size = 0;
	VTI("%s reset", (hw) ? "Hardware" : "Software");
	mutex_lock(&tcm_hcd->reset_mutex);
...
/* #ifndef	WAKEUP_GESTURE	*/	/*Either power off or enter sleep*/
	if (TOUCHSCREEN_GESTURE == vivoTsGetState()) {
...
static int syna_tcm_suspend(struct device *dev)
{
	...
	if (TOUCHSCREEN_SLEEP == vivoTsGetState()) {
...
static int syna_tcm_fb_notifier_cb(struct notifier_block *self,
				 unsigned long event, void *data)
{
	...
	int id = 0;
	struct syna_tcm_hcd *tcm_hcd =
		container_of(self, struct syna_tcm_hcd, fb_notifier);
	...
	if (tcm_hcd && event == 1) {
		...
		} else if (*blank == 1 && fb_suspend_flag == 0) {
			VTI("blank=%d event=%ld", *blank, event);
			fb_suspend_flag = 1;
			VTI("----------------------------called suspend");
			retval = syna_tcm_suspend(&tcm_hcd->pdev->dev);
	...
	if (tcm_hcd && event == 2) {
		...
		if (*blank == 1 && fb_suspend_flag == 0) {
			VTI("--------------------called early suspend!");
			retval = syna_tcm_early_suspend(&tcm_hcd->pdev->dev);
		} else if (*blank == 0 && fb_suspend_flag == 1) {
			VTI("--------------------called early resume!");
...
static int syna_irq_register(struct syna_tcm_hcd *tcm_hcd)
{
	int retval = 0;
	const struct syna_tcm_board_data *bdata = tcm_hcd->hw_if->bdata;

	if (bdata->irq_gpio < 0) {
		VTE("Invalid IRQ GPIO");
		retval = -EINVAL;
		goto queue_polling_work;
	}
	
	retval = vivoTsInterruptRegister(tcm_hcd->irq, NULL,
					syna_tcm_isr, IRQF_TRIGGER_LOW, /* 0x08 bdata->irq_flags,*/
					tcm_hcd);
	if (retval < 0) {
		VTE("Failed to create interrupt thread");
	} else {
		disable_irq(tcm_hcd->irq);
		VTI("request irq success");
	}
...
static int report_lcm_id(void)
{
	int lcm_id;

	lcm_id = (int)mdss_report_lcm_id();
...
static int qup_i2c_suspended = 0;
static int get_qcom_iic_status(void)
{
	return qup_i2c_suspended;
}

static int tcm_module_id(void)
{
	/*just for display in *#225# view*/
	if (vivoTsGetLcmId() == 0) {
		if (vivoTsGetVtsData()->getTsPinValue() == 0) {
			return VTS_MVC_JDI;	/* 帝晶+JDI */
		} else if (vivoTsGetVtsData()->getTsPinValue() > 0) {
			return VTS_MVC_JDI + 1;
		} else {
			return VTS_MVC_JDI + 2;	/* 帝晶+JDI */
		}
	}

	return VTS_MVC_JDI; /* default 帝晶+JDI */
...
extern struct spi_device *spi_client;
struct syna_tcm_hcd *tcm_hcd_gl;
static int syna_tcm_probe(struct platform_device *pdev)
{
	...
	struct vivo_ts_struct *vivo_ts_data = NULL;
	//struct spi_device *client = container_of(pdev->dev.parent, struct spi_device, dev);
	//int retry = 0;
	...
	if (spi_client == NULL) {
		VTE("Failed to get spi_client");
		return -EINVAL;
	}

	vivo_ts_data = vivoTsAlloc();
	if (!vivo_ts_data) {
		VTE("Alloc failed");
		return -ENOMEM;
	}

	vivo_ts_data->busType = VTS_BUS_SPI;
	vivo_ts_data->mtkOrQcom = VTS_PLATFORM_MTK_NEW;
	vivo_ts_data->icNum = VTS_IC_TD4330_N;
	vivo_ts_data->tsDimensionX = 1080;
	vivo_ts_data->tsDimensionY = 2340;
	vivo_ts_data->spiClient = spi_client;
	vivo_ts_data->rstGpio = bdata->reset_gpio;
	vivo_ts_data->irqGpio = bdata->irq_gpio;
	vivo_ts_data->v33Gpio = bdata->power_gpio;
	vivoTsSetTxRxNum(16, 36);
	vivo_ts_data->getLcmId = bbk_xxx_get_lcm_id;
	vivo_ts_data->getI2cBusState = get_qcom_iic_status;
	vivo_ts_data->getModuleId = tcm_module_id;
	vivo_ts_data->proFcFilterSwitch = 1;
	//vivo_ts_data->callbackType = 1;
	vivo_ts_data->irqEnableOrDisable = syna_tcm_enable_or_disable_irq;
	vivo_ts_data->imeiWriteSupport = UNSUPPORT;
/*
	vivoTsFwAdd(VTS_FW_TYPE_FW, PD1730F_EX_fw, sizeof(PD1730F_EX_fw), "PD1730F_EX", 0x00, VTS_MVC_SAM, 0x30);
	vivoTsFwAdd(VTS_FW_TYPE_FW, PD1730F_EX_fw_new, sizeof(PD1730F_EX_fw_new), "PD1730F_EX", 0x01, VTS_MVC_SAM, 0x30);
	vivoTsFwAdd(VTS_FW_TYPE_FW, PD1730F_EX_fw, sizeof(PD1730F_EX_fw), "PD1730F_EX", 0x00, VTS_MVC_SAM, 0x33);
	vivoTsFwAdd(VTS_FW_TYPE_FW, PD1730F_EX_fw_new, sizeof(PD1730F_EX_fw_new), "PD1730F_EX", 0x01, VTS_MVC_SAM, 0x33);
	vivoTsFwAdd(VTS_FW_TYPE_FW, PD1730F_EX_fw, sizeof(PD1730F_EX_fw), "PD1730F_EX", 0x00, VTS_MVC_SAM, 0x36);
	vivoTsFwAdd(VTS_FW_TYPE_FW, PD1730F_EX_fw_new, sizeof(PD1730F_EX_fw_new), "PD1730F_EX", 0x01, VTS_MVC_SAM, 0x36);
*/
	vivoTsFwAdd(VTS_FW_TYPE_FW, PD1831F_EX_vts_fw, sizeof(PD1831F_EX_vts_fw), "PD1831F_EX", VTS_FW_NOPIN_VALUE, VTS_MVC_JDI, 0x40);

	retval = vivoTsInit((void*)spi_client, NULL, 0);
	if (retval) {
		VTE("Alloc failed");
		goto vivo_ts_init_failed;
	}
	vivo_ts_data->suspendEventBlank = FB_EVENT_BLANK;
	vivo_ts_data->resumeEventBlank = FB_EVENT_BLANK;
	...
	atomic_set(&tcm_hcd->current_mode, TOUCHSCREEN_NORMAL);
...
#if 0
#ifdef CONFIG_FB
	VTI("-------------register notifier call_back!");
	...
	retval = syna_irq_register(tcm_hcd);
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
...
int bbk_sw_reset(void)
{
	/*return 0;*/
	return syna_tcm_reset(tcm_hcd_gl, false, false);
}
...
static void vivo_ts_core_module_init(void)
{
	int retval;
	retval = syna_tcm_bus_init();
	if (retval < 0) {
		VTI("syna tcm bus init fail");
	}

	platform_driver_register(&syna_tcm_driver);
}

/*extern unsigned int power_off_charging_mode;*/

//extern unsigned int is_atboot;
static unsigned int is_atboot = 0;
...
static int __init syna_tcm_module_init(void)
{
	...
	VTI("--------syna_tcm_module_init start!");

	retval = bbk_xxx_get_lcm_id();
	if (retval != 0x40) {
		VTI("not spi module");
		return 0;	
	}
	VTI("TD4330 0-flash module lcm id is 0x%x", retval);
	...
	return vivo_touchscreen_new_module_init(vivo_ts_core_module_init, "vivo_ts_core_module_init");
}
