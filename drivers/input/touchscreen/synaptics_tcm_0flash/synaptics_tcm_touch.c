...
struct touch_hcd {
	...
	struct mutex sync_report_mutex;
...
static int touch_parse_report(void)
{
	...
	unsigned int gesture_x;
	unsigned int gesture_y = 0;
#endif
		...
		case TOUCH_GESTURE_COORDINATE:
			...
			if (touch_data->gesture_report) {
				...
				for (gesture_coor_idx = 0; gesture_coor_idx < (bits / 32); ++gesture_coor_idx) {
					...
					VTI("(%d, %d)", gesture_x, gesture_y);
					if (gesture_coor_idx < 1) {
						vtsGesturePointsReport(VTS_GESTURE_POINT, gesture_coor_idx, gesture_x, gesture_y);
					} else if (gesture_coor_idx == 1) {
						vtsGesturePointsReport(VTS_GESTURE_POINT, ((bits / 32) - 1), gesture_x, gesture_y);
					} else {
						vtsGesturePointsReport(VTS_GESTURE_POINT, gesture_coor_idx - 1, gesture_x, gesture_y);
					}
				}
			}
...
static void touch_report(void)
{
	...
	VTD("gesture_report=%d, id = %02x, touchscreen_status=%d",
		...
		atomic_read(&tcm_hcd->current_mode));

#ifdef WAKEUP_GESTURE
#if 0
	if (touch_data->gesture_report && tcm_hcd->in_suspend) {
#else
	if (touch_data->gesture_report && (TOUCHSCREEN_GESTURE == atomic_read(&tcm_hcd->current_mode))) {
#endif
...
#else
	if (TOUCHSCREEN_NORMAL != atomic_read(&tcm_hcd->current_mode))
#endif
		goto exit;
	...
	for (idx = 0; idx < touch_hcd->max_objects; idx++) {
		...
		if (FINGER == status || GLOVED_FINGER == status) {
			touch_count++;
		}
	}
	retval = vivoTsCoverMute(touch_count, 0);
	if (retval < 0) {
		if (-2 == retval) {
...
#endif
#else
			vivoTsInputReport(VTS_TOUCH_UP, idx, 0, 0, 0);
#endif
			break;
		case FINGER:
		case GLOVED_FINGER:
...
#endif
#else
			vivoTsInputReport(VTS_TOUCH_DOWN, idx, x, y, (x + y) % 10 + 1);
#endif
			touch_count++;
			break;
		default:
...
static int touch_set_input_reporting(void)
{
	...
	if (tcm_hcd->er_disable_irq_flag) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"early_resume disable irq is open, no need to set input reporting\n");
		return 0;
	}

	mutex_lock(&touch_hcd->sync_report_mutex);
...
#ifdef WAKEUP_GESTURE
		if (TOUCHSCREEN_GESTURE == atomic_read(&tcm_hcd->current_mode)) {
...
exit:
	...
	mutex_unlock(&touch_hcd->sync_report_mutex);
...
static int touch_init(struct syna_tcm_hcd *tcm_hcd)
{
	...
	mutex_init(&touch_hcd->sync_report_mutex);
