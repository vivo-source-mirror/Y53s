...
#define FW_IMAGE_NAME "synaptics_hdl_firmware.img"
...
#define RESET_TO_HDL_DELAY_MS 1
...
static int zeroflash_caltimediff(struct timeval *diff, struct timeval *start, struct timeval *end)   
{
...
static int zeroflash_get_fw_image(void)
{
	...
	if (retval < 0) {
		LOGD(tcm_hcd->pdev->dev.parent,
				"Failed to request %s\n",
				FW_IMAGE_NAME);
		zeroflash_hcd->image = vivoTsGetFw(VTS_FW_TYPE_FW, &zeroflash_hcd->image_size);
	} else {
		VTI("request firmware success, Firmware image size = %d\n", (unsigned int)zeroflash_hcd->fw_entry->size);
...
static void zeroflash_download_config(void)
{
	...
	if (!fw_status->need_app_config && !fw_status->need_disp_config) {
		...
		atomic_set(&tcm_hcd->host_fw_valid, 1);
		wake_up_interruptible(&tcm_hcd->hdl_wq);
		do_gettimeofday(&end);
		zeroflash_caltimediff(&diff, &start, &end);
		VTI("Update firmware success! <%ld.%06ld>", diff.tv_sec, diff.tv_usec);
		if (zeroflash_hcd->fw_entry)
			release_firmware(zeroflash_hcd->fw_entry);
		zeroflash_hcd->fw_entry = NULL;
		return;
...
if (0) {
	...
	if ((tmp_buf[0] != MESSAGE_MARKER) && (tmp_buf[1] != REPORT_IDENTIFY)) {
		...
		if (retry_cnt <= 3) {
			...
			/* reset ic to retry download firmware */
			gpio_set_value(bdata->reset_gpio, bdata->reset_on_state);
			msleep(bdata->reset_active_ms);
			gpio_set_value(bdata->reset_gpio, !bdata->reset_on_state);
			mdelay(RESET_TO_HDL_DELAY_MS);
...
static void zeroflash_download_firmware_work(struct work_struct *work)
{
	...
	atomic_set(&zeroflash_hcd->workqueue_running, 1);
...
exit:
	...
	if (0) {
		...
		if (retval < 0) {
			//tcm_hcd->reset(tcm_hcd, true, false); //use the hw reset function on your platform
			gpio_set_value(bdata->reset_gpio, bdata->reset_on_state);
			msleep(1);
			gpio_set_value(bdata->reset_gpio, !bdata->reset_on_state);
			msleep(20); 
	...
	atomic_set(&zeroflash_hcd->workqueue_running, 0);
	return;
}
...
static int zeroflash_syncbox(struct syna_tcm_hcd *tcm_hcd)
{
	...
	case REPORT_STATUS:
		...
		break;
	case REPORT_HDL:
		if (atomic_read(&zeroflash_hcd->workqueue_running)) {
			LOGE(tcm_hcd->pdev->dev.parent,
					"host download is running return 0\n");
			return 0;
		}
		retval = tcm_hcd->enable_irq(tcm_hcd, false, true);
		if (retval < 0) {
