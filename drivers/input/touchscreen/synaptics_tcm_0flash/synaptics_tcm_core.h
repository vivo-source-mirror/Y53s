...
#define LOGD(dev, log, ...) 	do {	\
									VIVO_TS_LOG_DBG(log, ##__VA_ARGS__);		/*LOGx(dev_dbg, dev, log, ##__VA_ARGS__)*/\
								} while ((dev) == NULL)
#define LOGI(dev, log, ...) 	do {	\
									VIVO_TS_LOG_INF(log, ##__VA_ARGS__);		/*LOGx(dev_info, dev, log, ##__VA_ARGS__)*/\
								} while ((dev) == NULL)
#define LOGN(dev, log, ...) 	do {	\
									VIVO_TS_LOG_INF(log, ##__VA_ARGS__);		/*LOGx(dev_notice, dev, log, ##__VA_ARGS__)*/\
								} while ((dev) == NULL)
#define LOGW(dev, log, ...) 	do {	\
									VIVO_TS_LOG_INF(log, ##__VA_ARGS__);		/*LOGy(dev_warn, dev, log, ##__VA_ARGS__)*/\
								} while ((dev) == NULL)
#define LOGE(dev, log, ...) 	do {	\
									VIVO_TS_LOG_INF(log, ##__VA_ARGS__);		/*LOGy(dev_err, dev, log, ##__VA_ARGS__)*/\
								} while ((dev) == NULL)
...
struct syna_tcm_hcd {
	...
	bool er_disable_irq_flag;  /* early_resume disable_irq flag */
	...
	unsigned int bootloader_packrat_number;
...
struct syna_tcm_hw_interface {
	...
	#ifdef CONFIG_OF
	struct syna_tcm_pinctrl pinctrl;
	#endif
};
