...
static int parse_dt(struct device *dev, struct syna_tcm_board_data *bdata)
{
	...
	struct syna_tcm_pinctrl *pinctrl = &hw_if.pinctrl;

	pinctrl->pinctrl_gpios = devm_pinctrl_get(dev);
	if (IS_ERR(pinctrl->pinctrl_gpios)) {
		ret = PTR_ERR(pinctrl->pinctrl_gpios);
	...
	pinctrl->spi_cs_set = pinctrl_lookup_state(pinctrl->pinctrl_gpios, "spi_cs_set");
	if (IS_ERR(pinctrl->spi_cs_set))	{
		ret = PTR_ERR(pinctrl->spi_cs_set);
		VTI("spi_cs_set pinctrl");		
	}
	pinctrl_select_state(pinctrl->pinctrl_gpios, pinctrl->spi_cs_set);

	pinctrl->pin_cs_pulllow = pinctrl_lookup_state(pinctrl->pinctrl_gpios,  "spi_cs_pulllow");
	if (IS_ERR(pinctrl->pin_cs_pulllow))	{
		ret = PTR_ERR(pinctrl->pin_cs_pulllow);
		VTI("pin_cs_pulllow pinctrl");
	}
