...
#else
		reflash_hcd->image = vivoTsGetFw(VTS_FW_TYPE_FW, &reflash_hcd->image_size);
#endif
	}
...
static void vivo_ts_reflash_init(void)
{
	int ret = 0;
	ret = syna_tcm_add_module(&reflash_module, true);
	if (ret < 0) {
		VTI("add reflash modu fail");
	}
}
static int __init reflash_module_init(void)
{
	...
	if (0) {
		vivo_touchscreen_new_module_init(vivo_ts_reflash_init, "vivo_ts_reflash_init");
	}
