...
#define FIRST_APPLICATION_UID KUIDT_INIT(10000)
#define LAST_APPLICATION_UID  KUIDT_INIT(19999)
static __maybe_unused bool zram_wb_is_app_uid(void)
{
	kuid_t uid = current_uid();

	if (uid_gte(uid, FIRST_APPLICATION_UID) && uid_lte(uid, LAST_APPLICATION_UID))
		return true;
	return false;
}
#endif
		...
		struct list_head *uf)
{
	...
	if (current->group_leader) {
		vma->pid = current->tgid;
		/*
		 * pid == tgid: ui thread
		 */
		if (current->pid == current->tgid && zram_wb_is_app_uid())
			vma->pid = -2;
	} else {
		vma->pid = -1;
	}
