...
enum mt6785_afe_gpio {
	MT6785_AFE_GPIO_CLK_MOSI_OFF,
	MT6785_AFE_GPIO_CLK_MOSI_ON,
...
static struct audio_gpio_attr aud_gpios[MT6785_AFE_GPIO_GPIO_NUM] = {
	[MT6785_AFE_GPIO_CLK_MOSI_OFF] = {"aud_clk_mosi_off", false, NULL},
	[MT6785_AFE_GPIO_CLK_MOSI_ON] = {"aud_clk_mosi_on", false, NULL},
...
static int mt6785_afe_gpio_adda_clk(struct mtk_base_afe *afe, bool enable)
{
	static int counter;
	if (enable) {
		counter++;
		mt6785_afe_gpio_select(afe, MT6785_AFE_GPIO_CLK_MOSI_ON);
	} else {
		counter--;
		if (counter <= 0) {
			counter = 0;
			mt6785_afe_gpio_select(afe, MT6785_AFE_GPIO_CLK_MOSI_OFF);
		}
	}
	return 0;
}
			    ...
			    int dai, int uplink)
{
	...
	case MT6785_DAI_ADDA:
		mt6785_afe_gpio_adda_clk(afe, enable);
		if (uplink)
